...
const state = { view: 'dashboard', timelineFilter: 'all', rangeMs: 6 * 3600 * 1000, dateFilter: '' };
...
function getStorageSnapshot() {
  try { return window.etherx?.storage?.getSnapshot?.() || {}; } catch (_) { return {}; }
}
function getStoredJson(key, fallback) {
  ...
  try {
    ...
    const raw = snap && Object.prototype.hasOwnProperty.call(snap, key) ? snap[key] : null;
...
function getFilteredEvents(events) {
  ...
  if (!state.dateFilter) return list;
  return list.filter(ev => toDateKey(Number(ev?.ts || 0)) === state.dateFilter);
}
function getActiveEvents() {
  return getFilteredEvents(Array.isArray(liveSnapshot?.events) ? liveSnapshot.events : []);
}
...
function updateDateChip() {
  ...
  if (!el) return;
  if (state.dateFilter) {
    const d = new Date(state.dateFilter + 'T00:00:00');
    el.textContent = `Filter: ${d.toLocaleDateString('hr-HR', { year: 'numeric', month: 'short', day: 'numeric' })}`;
    return;
  }
  el.textContent = new Date().toLocaleDateString('hr-HR', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' });
}
...
function renderMetrics() {
  ...
  const uniqueUsers = new Set(activeEvents.map(ev => String(ev?.user || '').trim()).filter(Boolean)).size;
  const filteredCoins = activeEvents.reduce((sum, ev) => sum + Math.max(0, Number(ev?.coins || 0)), 0);
  ...
  const rows = [
    ...
    { icon: '✦', label: 'Gift Coins', value: liveOsFormat(state.dateFilter ? filteredCoins : ses.totalCoins), meta: `${liveOsFormat((liveSnapshot.gifts || []).length)} gift types` },
    { icon: '⟡', label: 'Events', value: liveOsFormat(state.dateFilter ? activeEvents.length : ses.messageCount), meta: state.dateFilter ? 'Filtered day' : 'Current session' },
    { icon: '♡', label: 'Users', value: liveOsFormat(state.dateFilter ? uniqueUsers : ses.uniqueUsers), meta: 'Unique' },
  ];
...
function renderGiftPanel() {
  ...
  const events = getActiveEvents();
  const gifts = buildGiftStats(events);
...
function renderSupportersPanel() {
  ...
  const events = getActiveEvents();
  const rows = showUsers ? buildUserStats(events) : buildTopSupporters(events);
...
function renderMusicPanel() {
  ...
  const history = getFilteredEvents(Array.isArray(music.history) ? music.history : []);
...
function updateStatusBar() {
  ...
  const eventsLabel = state.dateFilter ? activeEvents.length : (ses.messageCount || 0);
  bar.innerHTML = `<span class="status-dot ${isScanning ? 'online' : ''}"></span>${isScanning ? 'Live scanning' : 'Idle'}${conn.owner ? ` · @${liveosEsc(conn.owner)}` : ''}${eventsLabel ? ` · ${liveOsFormat(eventsLabel)} events` : ''}${state.dateFilter ? ` · ${liveosEsc(state.dateFilter)}` : ''}${lastSnapshotAt ? ` · Updated ${liveOsRelativeTime(lastSnapshotAt)}` : ''}`;
}

function loadSavedSessions() {
  const sessions = getStoredJson('ex_tkai_sessions', []);
...
async function loadBridgeSnapshot() {
  try {
    if (!window.etherx?.liveos?.getSnapshot) return false;
    ...
    if (snapshot && typeof snapshot === 'object') {
      ...
      if (hasEvents || hasSession) { applySnapshot(snapshot); return true; }
    }
  } catch (_) { }
  return false;
}
...
function tryLoadAutosave() {
  ...
  try {
    const session = getStoredJson('ex_tkai_live_autosave', null); if (!session?.messages?.length) return;
    applySnapshot({
...
document.addEventListener('click', ev => {
  ...
  if (ev.target.closest('#btnStartCapture')) { window.parent?.postMessage?.({ type: 'liveos:command', action: 'start-scan' }, '*'); window.parent?.postMessage?.({ type: 'liveos:command', action: 'startScan' }, '*'); window.liveos?.command?.('start-scan'); window.liveos?.command?.('startScan'); showToast('▶ Requested TikTok Chat AI start scan'); return; }
  if (ev.target.closest('#btnStopCapture')) { window.parent?.postMessage?.({ type: 'liveos:command', action: 'stop-scan' }, '*'); window.parent?.postMessage?.({ type: 'liveos:command', action: 'stopScan' }, '*'); window.liveos?.command?.('stop-scan'); window.liveos?.command?.('stopScan'); showToast('⏹ Requested scan stop'); return; }
  ...
  if (ev.target.closest('#btnPluginTools')) {
    const menu = document.getElementById('pluginToolsMenu');
    if (menu) menu.style.display = menu.style.display === 'block' ? 'none' : 'block';
    return;
  }
  if (ev.target.closest('[data-plugin-tools="reload-bridge"]')) {
    loadBridgeSnapshot().then(ok => { if (ok) showToast('Bridge snapshot loaded'); else showToast('No bridge snapshot available'); });
    document.getElementById('pluginToolsMenu')?.style && (document.getElementById('pluginToolsMenu').style.display = 'none');
    return;
  }
  if (ev.target.closest('[data-plugin-tools="load-latest-session"]')) {
    const sessions = loadSavedSessions();
    if (!sessions.length) { showToast('No saved sessions found'); return; }
    const latest = [...sessions].sort((a, b) => new Date(b.savedAt || b.exportedAt || 0) - new Date(a.savedAt || a.exportedAt || 0))[0];
    applySnapshot(sessionToSnapshot(latest));
    showToast('Loaded latest saved session');
    document.getElementById('pluginToolsMenu')?.style && (document.getElementById('pluginToolsMenu').style.display = 'none');
    return;
  }
  if (ev.target.closest('[data-plugin-tools="pick-date"]')) {
    document.getElementById('pluginToolsDateInput')?.click();
    return;
  }
  if (ev.target.closest('[data-plugin-tools="clear-date"]')) {
    state.dateFilter = '';
    updateDateChip();
    renderAllPanels();
  ...
  if (ev.target.closest('[data-plugin-tools="today"]')) {
    state.dateFilter = toDateKey(Date.now());
    updateDateChip();
    renderAllPanels();
    showToast('Filter: today');
    document.getElementById('pluginToolsMenu')?.style && (document.getElementById('pluginToolsMenu').style.display = 'none');
    return;
  }
  if (ev.target.closest('[data-plugin-tools="yesterday"]')) {
    state.dateFilter = toDateKey(Date.now() - 86400000);
    updateDateChip();
    renderAllPanels();
    showToast('Filter: yesterday');
    document.getElementById('pluginToolsMenu')?.style && (document.getElementById('pluginToolsMenu').style.display = 'none');
    return;
  }
  const toolsMenu = document.getElementById('pluginToolsMenu');
  if (toolsMenu && toolsMenu.style.display === 'block' && !ev.target.closest('#pluginToolsMenu')) toolsMenu.style.display = 'none';
});

document.getElementById('pluginToolsDateInput')?.addEventListener('change', (ev) => {
  const value = String(ev?.target?.value || '').trim();
  state.dateFilter = value;
  updateDateChip();
  renderAllPanels();
  showToast(value ? `Filter: ${value}` : 'Date filter cleared');
  const menu = document.getElementById('pluginToolsMenu');
  if (menu) menu.style.display = 'none';
});

setInterval(() => { loadBridgeSnapshot(); tryLoadAutosave(); updateStatusBar(); }, 5000);

document.addEventListener('DOMContentLoaded', () => {
  initDom();
  updateDateChip();
  loadBridgeSnapshot().finally(() => { tryLoadAutosave(); renderAllPanels(); });
  tryLoadAutosave();
  setView('dashboard');

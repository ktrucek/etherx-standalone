...
if (window.electronWebview) {
  const _wvEl =
    document.getElementById("browseFrameElectron") ||
...
const DB = {
  ...
  addBookmark(e) {
    ...
    if (b.find((x) => x.url === e.url)) {
      if (!e.silent) showToast("Already bookmarked");
      return;
    }
    b.unshift({ ...e, ts: Date.now() });
  ...
  removeBookmark(u) {
    localStorage.setItem(
      "ex_bm",
      JSON.stringify(this.getBookmarks().filter((x) => x.url !== u)),
    );
  },
  getSettings: () => {
    const now = Date.now();
  ...
  saveUser(u) {
    localStorage.setItem(
      "ex_user",
      JSON.stringify({ ...this.getUser(), ...u, updatedAt: Date.now() }),
    );
  },
  getNotes: () => JSON.parse(localStorage.getItem("ex_notes") || "[]"),
  addNote(n) {
    const arr = this.getNotes();
    arr.unshift({ id: Date.now(), ...n, ts: Date.now() });
    localStorage.setItem("ex_notes", JSON.stringify(arr.slice(0, 500)));
  },
  deleteNote(id) {
    localStorage.setItem(
      "ex_notes",
      JSON.stringify(this.getNotes().filter((n) => n.id !== id)),
    );
  },
  updateNote(id, data) {
    localStorage.setItem(
  ...
  addDownload(d) {
    ...
    arr.unshift({ id: Date.now(), ...d, ts: Date.now() });
  ...
  getStorageConfig: () =>
    JSON.parse(localStorage.getItem("ex_storage_cfg") || "{}"),
  getStorageKey(store) {
    ...
    return (
      cfg[store] ||
      {
        ...
        notes: "ex_notes",
        dl: "ex_dl",
        passwords: "ex_passwords",
        sessions: "ex_sessions",
      }[store] ||
      "ex_" + store
    );
  },
  setStorageKey(store, key) {
    const cfg = this.getStorageConfig();
    cfg[store] = key;
    localStorage.setItem("ex_storage_cfg", JSON.stringify(cfg));
    showToast('💾 Storage key for "' + store + '" set to "' + key + '"');
  },
};
let tabIdCounter = 0;
function createTabFrame(tabId, partition) {
  ...
  wv.addEventListener("did-start-loading", () => {
    if (STATE.activeTabId === tabId) setLoading(30);
  });
  wv.addEventListener("did-stop-loading", () => {
    if (STATE.activeTabId === tabId) setLoading(100);
  });
  wv.addEventListener("did-fail-load", (e) => {
    ...
    if (STATE.activeTabId === tabId) {
      ...
      if (t) showPageError(e.errorDescription || "Failed to load");
    }
  });
  wv.addEventListener("page-title-updated", (e) => {
    const t = STATE.tabs.find((x) => x.id === tabId);
  ...
  wv.addEventListener("did-navigate", (e) => {
    ...
    if (STATE.activeTabId === tabId) {
      const pb = document.getElementById("phishingBanner");
  ...
  wv.addEventListener("context-menu", (e) => {
    ...
    const img =
      e.params?.mediaType === "image" && e.params?.srcURL
        ? e.params.srcURL
        : null;
...
function createTab(url = "", title = "New Tab", active = true) {
  ...
  if (STATE.tabs.length >= MAX_TABS) {
    // Find first non-pinned tab and close it
    const toClose = STATE.tabs.find((t) => !t.pinned);
  ...
  if (url) {
    ...
    if (cfg.newTabWith === "Homepage" && cfg.homepage) {
      navigateTo(cfg.homepage, id);
    }
  }
  return tab;
}
function renderTab(tab) {
  const tabBar = document.getElementById("tabBar"),
    newBtn = document.getElementById("newTabBtn");
  const el = document.createElement("div");
  el.className =
    "tab" + (tab.pinned ? " pinned" : "") + (tab.muted ? " muted" : "");
  el.dataset.tabId = tab.id;
  el.draggable = true;
  const favHtml = tab.faviconUrl
    ? `<img src="${tab.faviconUrl}" onerror="this.parentNode.innerHTML='<span>🌐</span>'">`
...
function switchTab(id) {
  ...
  if (!tab.url) {
    ...
    if (window.electronWebview) {
      ...
      tabFrames.forEach((wv, tid) => {
        wv.style.display = tid === id ? "" : "none";
        wv.classList.toggle("active", tid === id);
      });
      frame.style.display = "none";
      frame.classList.remove("active");
...
function saveSessionTabs() {
  ...
  try {
    if (
      window.electronAPI &&
      typeof window.electronAPI.windowId === "function"
    ) {
...
function closeTab(id) {
  ...
  if (window.electronWebview) {
    const wv = tabFrames.get(id);
    if (wv) {
      wv.src = "about:blank";
      wv.remove();
      tabFrames.delete(id);
      tabFrameReady.delete(id);
    }
  }
  saveSessionTabs();
...
function updateTabEl(tab) {
  ...
  if (tab.faviconUrl) {
    fav.innerHTML = `<img src="${tab.faviconUrl}" onerror="this.parentNode.innerHTML='<span>🌐</span>'">`;
  } else {
    fav.innerHTML = `<span>${tab.favicon || "🌐"}</span>`;
  }
}
...
function navigateTo(raw, tabId) {
  ...
  const tab = tabId ? STATE.tabs.find((t) => t.id === tabId) : getActiveTab();
  if (!tab) return;
  tab.url = url;
  try {
  ...
  updateTabEl(tab);
  DB.addHistory({ url, title: tab.title });
  saveSessionTabs();
  consoleLog("info", "🌐 Navigating: " + url);
  logNetworkEntry(url);
}
function updateNavBtns(tab) {
  if (!tab) return;
  document.getElementById("btnBack").disabled = tab.histIdx <= 0;
  document.getElementById("btnFwd").disabled =
    tab.histIdx >= tab.history.length - 1;
}
document.getElementById("btnBack").addEventListener("click", () => {
  ...
  tab.histIdx--;
  const u = tab.history[tab.histIdx];
  tab.url = u;
  ...
  if (window.electronWebview) {
    ...
    if (wv) {
      safeWebviewExecute(wv, tab.id, "goBack").catch(() => {
        wv.src = u;
      });
    }
  } else {
...
document.getElementById("btnFwd").addEventListener("click", () => {
  ...
  if (window.electronWebview) {
    const wv = getTabWebview(tab.id);
    if (wv) {
      safeWebviewExecute(wv, tab.id, "goForward").catch(() => {
        wv.src = u;
      });
    }
  } else {
...
document.getElementById("btnReload").addEventListener("click", () => {
  ...
  if (tab && tab.url) {
    ...
    if (window.electronWebview) {
      const wv = getTabWebview(tab.id);
      if (wv) {
        safeWebviewExecute(wv, tab.id, "reload").catch(() => {
          wv.src = tab.url;
        });
      }
    } else {
...
function getUrlHistory() {
  try {
    return JSON.parse(localStorage.getItem(URL_HIST_KEY) || "[]");
  } catch (e) {
    return [];
  }
}
function saveUrlHistory(arr) {
  localStorage.setItem(
    URL_HIST_KEY,
    JSON.stringify(arr.slice(0, URL_HIST_MAX)),
  );
}
...
function highlightMatch(text, q) {
  ...
  return (
    escAcHtml(text.slice(0, idx)) +
    "<mark>" +
    escAcHtml(text.slice(idx, idx + q.length)) +
    "</mark>" +
    escAcHtml(text.slice(idx + q.length))
  );
}
  ...
  items.forEach((item, i) => {
    ...
    div.addEventListener("mousedown", (e) => {
      if (e.target.classList.contains("ac-del")) {
        ...
        e.stopPropagation();
        let h = getUrlHistory();
        h = h.filter((x) => x.url !== item.url);
        saveUrlHistory(h);
        items.splice(i, 1);
        renderAcDropdown(items);
        return;
      }
      e.preventDefault();
...
function queryAcDropdown(q) {
  ...
  const tabMatches = STATE.tabs
    ...
    .map((t) => ({
      ...
      favicon: t.faviconUrl
        ? `<img src="${escAcHtml(t.faviconUrl)}" style="width:14px;height:14px;border-radius:2px;object-fit:cover">`
        : "📄",
      type: "url",
    }))
    .slice(0, 3);
  ...
  // Deduplicate
  const seen = new Set();
  const combined = [...hist, ...tabMatches, ...bmMatches].filter((x) => {
  ...
  const items = [];
  if (looksLikeUrl && !seen.has(q))
    items.push({ url: q, title: "", favicon: "🌐", type: "url" });
  items.push(...combined);
...
if (urlInput) {
  urlInput.addEventListener("keydown", (e) => {
    ...
    if (e.key === "Escape") {
      urlInput.value = getActiveTab()?.url || "";
      closeAcDropdown();
      urlInput.blur();
      return;
    }
    if (e.key === "ArrowDown") {
      ...
      acDropdown
        .querySelectorAll(".url-ac-item")
        .forEach((el, i) => el.classList.toggle("ac-sel", i === _acSelIdx));
  ...
  urlInput.addEventListener("input", () => {
    _acSelIdx = -1;
    queryAcDropdown(urlInput.value.trim());
  });
  urlInput.addEventListener("focus", () => {
    urlInput.select();
    if (urlInput.value.trim()) queryAcDropdown(urlInput.value.trim());
  });
...
frame.addEventListener("load", () => {
  ...
  try {
    ...
    if (t && t !== "") {
      tab.title = t;
      document.getElementById("elemTitle").textContent = t;
    }
    ...
    `,
        ...
        .then((perf) => {
          if (!perf) return;
          const fcpMs = perf.fcp;
          const loadMs = perf.load;
          ...
          document.getElementById("perfTbt").textContent =
            perf.domInteractive != null ? perf.domInteractive + "ms" : "—";
          document.getElementById("perfCls").textContent = "—";
          document.getElementById("perfTti").textContent =
            perf.domInteractive != null ? perf.domInteractive + "ms" : "—";
          // Store in tab state for Performance Monitor panel
          if (STATE.tabs?.[tab.id])
            STATE.tabs[tab.id].perfData = {
              fcp: fcpMs,
              lcp: null,
              load: loadMs,
            };
...
window.addEventListener("message", (e) => {
  ...
  if (e.data?.type === "etherx-resources") {
    ...
    if (treeEl && d.resources) {
      ...
      const folderKey =
        "src-folder-" + (d.origin || "").replace(/[^a-z0-9]/gi, "_");
      if (!document.getElementById(folderKey)) {
        const folder = document.createElement("div");
        folder.className = "src-item folder";
        folder.style.paddingLeft = "20px";
        folder.id = folderKey;
        folder.textContent =
          "📁 " +
          (d.origin || "(loaded page)")
            .replace("https://", "")
            .replace("http://", "");
      ...
      d.resources.forEach((r) => {
        ...
        if (!document.getElementById(rKey)) {
          ...
          item.dataset.srcUrl = r.url;
          item.textContent = icon + " " + name.slice(0, 40);
          item.title = r.url;
          item.addEventListener("click", () => {
            document
              .querySelectorAll(".src-item")
...
function setZoom(val) {
  ...
  z.style.display = STATE.zoom !== 100 ? "block" : "none";
  z.textContent = STATE.zoom + "%";
  document.getElementById("sbZoom").textContent = STATE.zoom + "%";
}
document
  .getElementById("zoomIndicator")
    ...
    if (wv && typeof wv.findInPage === "function") {
      try {
        ...
        wv.addEventListener("found-in-page", function handler(e) {
          document.getElementById("findCount").textContent = e.result.matches
...
document.getElementById("readerSmaller").addEventListener("click", () => {
  readerFontSize = Math.max(12, readerFontSize - 2);
  document.querySelector(".reader-content").style.fontSize =
    readerFontSize + "px";
});
document.getElementById("readerLarger").addEventListener("click", () => {
...
function toggleReader() {
  ...
  if (STATE.readerMode) {
    ...
    document.getElementById("readerContent").innerHTML =
      '<p style="color:#888;font-style:italic">📖 Extracting readable content…</p>';
        ...
        .then((result) => {
          if (result?.ok && result.text) {
            ...
            document.getElementById("readerContent").innerHTML =
              ...
              '</h1><p style="color:#888">Could not extract readable content from this page.</p>';
          }
        })
        .catch(() => {
          document.getElementById("readerContent").innerHTML =
...
document
  .getElementById("mi-responsive")
  .addEventListener("click", toggleRespMode);
document
  .getElementById("mi-responsive-dev")
  .addEventListener("click", toggleRespMode);
...
document.querySelectorAll(".rp").forEach((btn) => {
  ...
  btn.addEventListener("click", () => {
    ...
    const w = btn.dataset.w,
      h = btn.dataset.h;
    const rf = document.getElementById("respFrame");
    rf.style.width = w + "px";
    rf.style.height = h + "px";
    document.getElementById("respSize").textContent = w + " × " + h;
    const tab = getActiveTab();
    if (tab?.url) rf.src = tab.url;
  });
});
function toggleRespMode() {
  STATE.respMode = !STATE.respMode;
  const bar = document.getElementById("respBar");
  const wrap = document.getElementById("respWrapper");
  bar.classList.toggle("show", STATE.respMode);
  if (STATE.respMode) {
    wrap.classList.add("active");
    const rf = document.getElementById("respFrame");
    rf.style.width = "375px";
    rf.style.height = "812px";
    const tab = getActiveTab();
    if (tab?.url) rf.src = tab.url;
    ntp.style.display = "none";
    frame.classList.remove("active");
    showToast("📐 Responsive Design Mode");
  } else {
    wrap.classList.remove("active");
    document.getElementById("respFrame").src = "";
    const tab = getActiveTab();
    if (tab?.url) showFrame(tab);
    else showNTP();
  }
}
function toggleTabOverview() {
  const ov = document.getElementById("tabOverview");
  const isOpen = ov.classList.toggle("show");
  if (isOpen) renderTabOverview();
}
...
document
  .getElementById("mi-tab-overview")
...
document.addEventListener("fullscreenchange", () =>
  showToast(
    document.fullscreenElement
      ? "⛶ Full Screen — F11 to exit"
      : "⊡ Exited Full Screen",
  ),
);
document.getElementById("mi-new-private").addEventListener("click", () => {
  STATE.isPrivate = !STATE.isPrivate;
  document.getElementById("privateIndicator").style.display = STATE.isPrivate
    ? "block"
    : "none";
  document.getElementById("sbPrivate").style.display = STATE.isPrivate
    ? "block"
    : "none";
  document.body.style.filter = STATE.isPrivate
    ? "hue-rotate(240deg) saturate(0.8)"
    : "";
...
document.getElementById("btnHistory").addEventListener("click", () => {
  renderHistoryPanel();
  togglePanel("histPanel");
});
document
  .getElementById("btnDownloads")
  .addEventListener("click", () => togglePanel("dlPanel"));
...
document.getElementById("walletReload")?.addEventListener("click", () => {
  ...
  if (wf) {
    wf.src = "";
    setTimeout(() => {
      wf.src = "https://wallet.kriptoentuzijasti.io";
    }, 50);
  }
});
document.getElementById("btnWallet").addEventListener("click", () => {
  togglePanel("walletPanel");
  window._wltInit && window._wltInit();
});
document
  .getElementById("btnBobiAI")
  .addEventListener("click", () => togglePanel("bobiaiPanel"));
...
async function summarizeCurrentPage() {
  ...
  if (!card || !loader || !bulletsEl)
    return { ok: false, error: "AI summary panel not found in DOM" };
  ...
  if (window.electronWebview) {
    try {
      ...
      if (!html || html.length < 200) {
        ...
        _renderSummaryError(
          bulletsEl,
          "Not enough page content to summarize. Please wait for the page to fully load.",
        );
        bulletsEl.style.display = "block";
  ...
  try {
    ...
    if (!GEMINI_ENDPOINT) {
      ...
      _renderSummaryError(
        bulletsEl,
        "Gemini API ključ nije postavljen. Idi u Postavke → AI i unesi API ključ.",
      );
      bulletsEl.style.display = "block";
    ...
    const summary =
      apiData.candidates?.[0]?.content?.parts?.[0]?.text?.trim() || "";
...
function _renderSummaryBullets(el, bullets) {
  ...
  if (!bullets || !bullets.length) {
    el.innerHTML =
      '<div style="color:var(--text3);font-size:12px">Nema sažetka.</div>';
    return;
  }
  bullets.forEach((b, i) => {
...
document
  .getElementById("btnAiAgent")
  .addEventListener("click", () => togglePanel("aiAgentPanel"));
document
  .getElementById("btnTikTokAI")
  ?.addEventListener("click", () => togglePanel("tiktokAIPanel"));
...
function renderBookmarksPanel() {
  ...
  if (bm.length === 0) {
    list.innerHTML =
      '<div style="text-align:center;padding:32px;color:var(--text3);font-size:12px">No bookmarks yet<br><br>Press Ctrl+D to bookmark the current page</div>';
    return;
  }
  list.innerHTML = "";
  const title = document.createElement("div");
  title.className = "p-section-title";
  ...
  bm.forEach((b) => {
    ...
    el.querySelector(".p-entry-title").addEventListener("click", () => {
      navigateTo(b.url);
      closeAllPanels();
    });
    el.querySelector(".p-entry-del").addEventListener("click", (e) => {
      e.stopPropagation();
      DB.removeBookmark(b.url);
      renderBookmarksPanel();
    });
    list.appendChild(el);
  });
...
function renderHistoryPanel() {
  ...
  if (hist.length === 0) {
    list.innerHTML =
      '<div style="text-align:center;padding:32px;color:var(--text3);font-size:12px">No history yet</div>';
    return;
  }
  list.innerHTML = "";
  const groups = {};
...
function toggleDevtools() {
  ...
  document
    ...
    .classList.toggle("open", STATE.devtoolsOpen);
}
...
(function initPerfMon() {
  ...
  function drawSparkline(canvasId, data, color, maxVal) {
    ...
    grad.addColorStop(
      ...
      color
        ...
        .replace("rgb", "rgba")
        .replace("#", "rgba(")
        .replace("rgba(", color.startsWith("#") ? "rgba(" : "rgba("),
    );
  ...
  async function tick() {
    ...
    if (totalRamEl) {
      totalRamEl.textContent = fmt(ramVal);
      totalRamEl.className =
        "pm-card-val" + (ramVal > 2000 ? " bad" : ramVal > 1000 ? " warn" : "");
    }
    if (totalCpuEl) {
      totalCpuEl.textContent = cpuVal + "%";
      totalCpuEl.className =
        "pm-card-val" + (cpuVal > 80 ? " bad" : cpuVal > 50 ? " warn" : "");
    }
    const uptimeEl = document.getElementById("pmUptime");
    if (uptimeEl) uptimeEl.textContent = fmtSec(metrics.uptime);

    // Peak labels
    const rp = document.getElementById("pmRamPeak");
    if (rp) rp.textContent = "peak " + fmt(peakRam);
    const cp = document.getElementById("pmCpuPeak");
    if (cp) cp.textContent = "peak " + peakCpu + "%";
    ...
    if (heapMem) {
      ...
      const pct = Math.min(
        100,
        (heapMem.usedJSHeapSize / heapMem.jsHeapSizeLimit) * 100,
      ).toFixed(0);
      const hu = document.getElementById("pmHeapUsed");
      if (hu) hu.textContent = used + " MB";
      const ht = document.getElementById("pmHeapTotal");
      if (ht) ht.textContent = total + " MB";
      const hb = document.getElementById("pmHeapBar");
    ...
    if (pl && metrics.processes) {
      pl.innerHTML =
        '<div class="pm-proc-row" style="color:#666;font-size:9px;font-weight:600"><span>TYPE</span><span>PID</span><span style="text-align:right">RAM</span><span style="text-align:right">CPU</span></div>' +
        metrics.processes
          .map(
            (p) =>
    ...
    if (tab?.perfData) {
      if (pmFcp)
        pmFcp.textContent =
          tab.perfData.fcp != null ? tab.perfData.fcp + " ms" : "—";
      if (pmLcp)
        pmLcp.textContent =
          tab.perfData.lcp != null ? tab.perfData.lcp + " ms" : "—";
      if (pmLoad)
        pmLoad.textContent =
          tab.perfData.load != null ? tab.perfData.load + " ms" : "—";
    } else {
  ...
  if (pauseBtn) {
    pauseBtn.addEventListener("click", () => {
      ...
      document.getElementById("pmLiveIndicator").style.animationName = pmPaused
        ? "none"
        : "pmBlink";
      document.getElementById("pmLiveIndicator").textContent = pmPaused
        ? "● PAUSED"
        : "● LIVE";
      document.getElementById("pmLiveIndicator").style.color = pmPaused
        ? "#888"
        : "#27c93f";
    });
  }
...
document.querySelectorAll(".dt-tab").forEach((tab) => {
  tab.addEventListener("click", () => {
    ...
    if (!STATE.devtoolsOpen) {
      ...
      document.getElementById("devtools").classList.add("open");
    }
...
(function () {
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    dt.style.height =
      Math.min(Math.max(startH + delta, 120), window.innerHeight * 0.85) + "px";
  });
...
function updateDtBadges() {
  const eb = document.getElementById("dtErrBadge");
  const wb = document.getElementById("dtWarnBadge");
  if (eb) eb.textContent = dtErrCount || "";
  if (wb) wb.textContent = dtWarnCount || "";
}
...
(function hookBrowserConsole() {
  ...
  function hookFrameConsole() {
    ...
    try {
      ...
      if (!win) return;
      const origFLog = win.console.log,
        origFWarn = win.console.warn,
        origFError = win.console.error;
      win.console.log = function (...args) {
        ...
        origFLog.apply(win.console, args);
      };
      win.console.warn = function (...args) {
        ...
        origFWarn.apply(win.console, args);
      };
      win.console.error = function (...args) {
        ...
        origFError.apply(win.console, args);
      };
      win.addEventListener("error", (e) =>
        consoleLog(
          "error",
          `Page: ${e.message} at ${e.filename}:${e.lineno}`,
          "page",
        ),
      );
      win.addEventListener("unhandledrejection", (e) =>
        consoleLog(
          "error",
          "Page Promise Rejection: " + (e.reason?.message || e.reason),
          "page",
        ),
      );
    } catch (e) {
      /* cross-origin, can't hook */
    }
  }
  const fr = document.getElementById("browseFrame");
  if (fr) {
    fr.addEventListener("load", () => setTimeout(hookFrameConsole, 100));
    fr.addEventListener("did-finish-load", () =>
      setTimeout(hookFrameConsole, 100),
    );
  }
})();
...
document.getElementById("consoleInput").addEventListener("keydown", (e) => {
  ...
  consoleLog(
    "log",
    '<span style="color:#858585">❯ </span>' + escHtml(code),
    "console",
  );
  try {
    const r = eval(code);
    if (r !== undefined)
      consoleLog("info", "← " + JSON.stringify(r, null, 1), "<eval>:1");
  } catch (err) {
    consoleLog("error", err.message, "<eval>:1");
  }
  e.target.value = "";
});
...
document.querySelectorAll(".con-lvl").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    dtLevelFilter[lvl] = !dtLevelFilter[lvl];
    btn.classList.toggle("on", dtLevelFilter[lvl]);
    applyConsoleFilter();
  });
});
const conFilterEl = document.getElementById("conFilter");
if (conFilterEl)
  conFilterEl.addEventListener("input", (e) => {
    dtTextFilter = e.target.value.toLowerCase();
    applyConsoleFilter();
  });
function applyConsoleFilter() {
  document.querySelectorAll("#consoleOutput .cl").forEach((el) => {
    ...
    const type =
      [...el.classList].find((c) =>
        ["log", "info", "warn", "error", "success", "verbose"].includes(c),
      ) || "log";
...
function inspectMouseMove(e) {
  ...
  if (
    !target ||
    target.id === "inspectOverlay" ||
    target.id === "inspectTooltip" ||
...
document
  .getElementById("elemRefreshBtn")
  ?.addEventListener("click", renderDOMTree);
...
function renderDOMTree() {
  ...
  if (!view) return;
  view.innerHTML =
    '<div style="color:#888;padding:8px">Loading DOM tree...</div>';
  setTimeout(() => {
    try {
      const doc =
      ...
      if (
        ...
        !e.message?.includes("named property")
      ) {
...
function renderNode(node, depth) {
  ...
  const attrs = Array.from(node.attributes || [])
    .map(
      (a) =>
        ` <span class="el-attr">${escHtml(a.name)}</span>=<span class="el-str">"${escHtml(a.value.slice(0, 40))}${a.value.length > 40 ? "..." : ""}"</span>`,
    )
    .join("");
  const hasChildren =
  ...
  el.onclick = (e) => {
    e.stopPropagation();
    document
      .querySelectorAll(".el-node")
      .forEach((n) => n.classList.remove("el-selected"));
    ...
    if (hasChildren && el.querySelector(".el-arrow")) {
      ...
      while (
        ...
        parseInt(nextEl.style.paddingLeft) > parseInt(el.style.paddingLeft)
      ) {
...
function selectElement(elem) {
  ...
  try {
    ...
    if (fullEl) {
      const all = Array.from(computed)
        .sort()
        .map(
          (prop) =>
            `<div><strong>${prop}:</strong> ${computed.getPropertyValue(prop)}</div>`,
        )
        .join("");
    ...
    if (posEl)
      posEl.textContent = `position: ${computed.position} | top: ${computed.top} | left: ${computed.left} | z-index: ${computed.zIndex}`;

    // Events tab
    const eventsEl = document.getElementById("elemEvents");
    if (eventsEl) {
      eventsEl.innerHTML =
        '<div style="color:#888">Event listeners are not accessible in this browser implementation</div>';
    }
  } catch (e) {
    consoleLog(
      "error",
      "Failed to compute element styles: " + e.message,
      "Elements",
    );
  }
}
...
function logNetworkEntry(url, type, status, size, time) {
  ...
  const t = type || "HTML";
  const s = status || 200;
  const sz = size || Math.floor(Math.random() * 200 + 10) + "KB";
  const ms = time || Math.floor(Math.random() * 200 + 20) + "ms";
  const badgeClass =
    {
      HTML: "b-html",
      JS: "b-js",
...
document.getElementById("netClear")?.addEventListener("click", () => {
  if (networkLog) networkLog.innerHTML = "";
});
document.querySelectorAll(".net-type-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    document
      .querySelectorAll(".net-type-btn")
      .forEach((b) => b.classList.remove("on"));
    btn.classList.add("on");
    document.querySelectorAll("#networkLog tr").forEach((tr) => {
      tr.style.display =
        btn.dataset.ntype === "All" || tr.dataset.type === btn.dataset.ntype
          ? ""
          : "none";
    });
  });
...
const _appMap = {
  ...
  appSessionStorage: "sessionstorage",
  appCookies: "cookies",
  appIndexedDB: "indexeddb",
  appCacheStorage: "cache",
  appServiceWorkers: "sw",
  appSqliteHistory: "sqlite-history",
  appSqliteTabs: "sqlite-tabs",
  appSqliteBookmarks: "sqlite-bookmarks",
  appSqliteAiCache: "sqlite-aicache",
  appSqliteSettings: "sqlite-settings",
  appSqliteUser: "sqlite-user",
  appSqliteNotes: "sqlite-notes",
  appSqliteDownloads: "sqlite-downloads",
  appSqliteSessions: "sqlite-sessions",
  appSqlitePasswords: "sqlite-passwords",
};
...
function renderAppTab(type) {
  ...
  if (type === "localstorage") {
    ...
    if (!keys.length) {
      c.innerHTML =
        '<p style="color:#555;font-size:11px;padding:8px">No local storage data.</p>';
      return;
    }
    c.innerHTML =
      '<table class="app-table"><thead><tr><th>Key</th><th>Value</th></tr></thead><tbody>' +
      keys
        .map((k) => {
          let v = localStorage.getItem(k) || "";
          if (v.length > 120) v = v.slice(0, 120) + "…";
          return `<tr><td>${escHtml(k)}</td><td>${escHtml(v)}</td></tr>`;
        })
        .join("") +
      "</tbody></table>";
    ...
    if (!keys.length) {
      c.innerHTML =
        '<p style="color:#555;font-size:11px;padding:8px">No session storage data.</p>';
      return;
    }
    c.innerHTML =
      '<table class="app-table"><thead><tr><th>Key</th><th>Value</th></tr></thead><tbody>' +
      keys
        .map(
          (k) =>
            `<tr><td>${escHtml(k)}</td><td>${escHtml(sessionStorage.getItem(k) || "")}</td></tr>`,
        )
        .join("") +
      "</tbody></table>";
    ...
    if (!document.cookie) {
      c.innerHTML =
        '<p style="color:#555;font-size:11px;padding:8px">No cookies.</p>';
      return;
    }
    c.innerHTML =
      '<table class="app-table"><thead><tr><th>Name</th><th>Value</th></tr></thead><tbody>' +
      document.cookie
        .split(";")
        .map((p) => {
          const [k, ...v] = p.trim().split("=");
          return `<tr><td>${escHtml(k)}</td><td>${escHtml(v.join("="))}</td></tr>`;
        })
        .join("") +
      "</tbody></table>";
    ...
    if (!rows.length) {
      c.innerHTML = _sqliteEmpty("history");
      return;
    }
    c.innerHTML =
      `<div style="padding:8px 10px;font-size:10px;color:#858585;border-bottom:1px solid var(--border)">SQLite • etherx.db • table: <strong>history</strong> • ${rows.length} rows</div>` +
      '<table class="app-table"><thead><tr><th>#</th><th>URL</th><th>Title</th><th>Visits</th><th>Last Visited</th></tr></thead><tbody>' +
      rows
        .slice(0, 200)
        .map((r, i) => {
          const ts = r.ts || r.last_visited;
          const date = ts
            ? new Date(ts < 1e12 ? ts * 1000 : ts).toLocaleString("hr-HR")
            : "";
          const url = r.url || "";
    ...
    const fetchTabs =
      window.electronWebview && window.etherx?.tabs?.getAll
        ? window.etherx.tabs.getAll()
        : Promise.resolve(STATE.tabs);
    fetchTabs.then((rows) => {
      if (!rows || !rows.length) {
        c.innerHTML = _sqliteEmpty("open_tabs");
        return;
      }
      c.innerHTML =
        `<div style="padding:8px 10px;font-size:10px;color:#858585;border-bottom:1px solid var(--border)">SQLite • etherx.db • table: <strong>tabs</strong> • ${rows.length} rows</div>` +
    ...
    const fetchBm =
      window.electronWebview && window.etherx?.bookmarks?.getAll
        ? window.etherx.bookmarks.getAll()
        : Promise.resolve(DB.getBookmarks());
    fetchBm.then((rows) => {
      if (!rows || !rows.length) {
        c.innerHTML = _sqliteEmpty("bookmarks");
        return;
      }
      c.innerHTML =
        `<div style="padding:8px 10px;font-size:10px;color:#858585;border-bottom:1px solid var(--border)">SQLite • etherx.db • table: <strong>bookmarks</strong> • ${rows.length} rows</div>` +
        ...
        rows
          .map((r, i) => {
            ...
            const date = ts
              ? new Date(ts < 1e12 ? ts * 1000 : ts).toLocaleDateString("hr-HR")
              : "";
    ...
    fetchCache.then((rows) => {
      ...
      c.innerHTML =
        `<div style="padding:8px 10px;font-size:10px;color:#858585;border-bottom:1px solid var(--border)">SQLite • etherx.db • table: <strong>ai_cache</strong> • ${rows.length} rows • <button onclick="(window.etherx?.ai?.clearAiCache?.() || (_summaryCache && Object.keys(_summaryCache).forEach(k=>delete _summaryCache[k])));renderAppTab('sqlite-aicache')" style="background:#c0392b;border:none;color:#fff;border-radius:3px;cursor:pointer;font-size:10px;padding:1px 6px">Clear Cache</button></div>` +
        '<table class="app-table"><thead><tr><th>#</th><th>URL</th><th>Summary</th><th>Model</th><th>Cached</th></tr></thead><tbody>' +
        rows
          .map((r, i) => {
            const url = r.url || "";
            const short = url.length > 35 ? url.slice(0, 35) + "…" : url;
            const sum =
    ...
    const fetchSettings =
      window.electronWebview && window.etherx?.settings?.get
    ...
    fetchSettings.then((obj) => {
      ...
      if (!entries.length) {
        c.innerHTML = _sqliteEmpty("settings");
        return;
      }
      const HIDDEN = ["gemini_api_key", "openai_api_key", "api_key"];
      c.innerHTML =
        `<div style="padding:8px 10px;font-size:10px;color:#858585;border-bottom:1px solid var(--border)">SQLite • etherx.db • table: <strong>settings</strong> • ${entries.length} rows
        ...
        entries
          .map(([k, v]) => {
            const display = HIDDEN.some((h) => k.includes(h))
              ? "•••••••••• (hidden)"
              : escHtml(String(v).slice(0, 100));
    ...
    c.innerHTML =
      `<div style="padding:8px 10px;font-size:10px;color:#858585;border-bottom:1px solid var(--border)">SQLite • etherx.db • table: <strong>notes</strong> • ${notes.length} rows
    ...
    if (!rows.length) {
      c.innerHTML = _sqliteEmpty("downloads");
      return;
    }
    c.innerHTML =
      `<div style="padding:8px 10px;font-size:10px;color:#858585;border-bottom:1px solid var(--border)">SQLite • etherx.db • table: <strong>downloads</strong> • ${rows.length} rows</div>` +
    ...
    if (!rows.length) {
      c.innerHTML = _sqliteEmpty("sessions");
      return;
    }
    c.innerHTML =
      `<div style="padding:8px 10px;font-size:10px;color:#858585;border-bottom:1px solid var(--border)">SQLite • etherx.db • table: <strong>sessions</strong> • ${rows.length} saved sessions
      ...
      rows
        .map((r, i) => {
          ...
          const preview =
            (r.tabs || [])
              .slice(0, 3)
              .map((t) => escHtml((t.title || t.url || "?").slice(0, 20)))
              .join(", ") + (r.tabs?.length > 3 ? "…" : "");
    ...
    if (!rows.length) {
      c.innerHTML = _sqliteEmpty("passwords");
      return;
    }
    c.innerHTML =
      `<div style="padding:8px 10px;font-size:10px;color:#858585;border-bottom:1px solid var(--border)">SQLite • etherx.db • table: <strong>passwords</strong> • ${rows.length} entries <span style="color:#e74c3c">(passwords are masked)</span></div>` +
    ...
    c.innerHTML =
      '<p style="color:#555;font-size:11px;padding:8px">No data available.</p>';
  }
}
function _sqliteLoading(name) {
  return `<div style="padding:16px;color:#858585;font-size:12px;display:flex;align-items:center;gap:8px"><div style="width:14px;height:14px;border:2px solid #555;border-top-color:#4a9eff;border-radius:50%;animation:spin .8s linear infinite"></div>Loading ${name} from SQLite…</div>`;
}
function _sqliteEmpty(table) {
  return `<p style="color:#555;font-size:11px;padding:12px">🗄️ Table <strong>${escHtml(table)}</strong> is empty.</p>`;
}
function escHtml(s) {
  return String(s)
    .replace(/&/g, "&amp;")
...
function _editSetting(k) {
  ...
  const cur = HIDDEN.some((h) => k.includes(h))
    ? ""
    : s[k] !== undefined
      ? String(s[k])
      : "";
  ...
  else if (!isNaN(nv) && nv !== "") parsed = Number(nv);
...
function _addSetting() {
  ...
  if (!k) {
    showToast("⚠️ Key cannot be empty");
    return;
  }
  let parsed = v;
  if (v === "true") parsed = true;
  else if (v === "false") parsed = false;
  else if (!isNaN(v) && v !== "") parsed = Number(v);
  DB.saveSetting(k, parsed);
  renderAppTab("sqlite-settings");
...
function _changeUserAvatar() {
  const nv = prompt(
    "Enter emoji or text for avatar:",
    DB.getUser().avatar || "👤",
  );
  if (nv === null) return;
...
function _addNote() {
  ...
  if (!t) {
    showToast("⚠️ Note title cannot be empty");
    return;
  }
...
function _restoreSession(id) {
  ...
  if (!sess || !sess.tabs?.length) {
    showToast("⚠️ Session is empty");
    return;
  }
  if (
    !confirm(
...
function collectPerfMetrics() {
  try {
    ...
    if (nav) {
      ...
      const el = document.getElementById("perfLoad");
      if (el) el.textContent = load + "ms";
      const et = document.getElementById("perfTti");
      if (et) et.textContent = tti + "ms";
    }
    performance.getEntriesByType("paint").forEach((p) => {
      if (p.name === "first-contentful-paint") {
        const el = document.getElementById("perfFcp");
...
document
  .getElementById("mi-close-tab")
  .addEventListener("click", () => closeTab(STATE.activeTabId));
document.getElementById("mi-find").addEventListener("click", openFind);
document
  .getElementById("mi-reload")
  .addEventListener("click", () =>
    document.getElementById("btnReload").click(),
  );
document.getElementById("mi-hard-reload").addEventListener("click", () => {
  const t = getActiveTab();
  if (t?.url) {
    frame.src = "";
    setTimeout(() => {
...
document
  .getElementById("mi-zoom-in")
  .addEventListener("click", () => setZoom(STATE.zoom + 10));
document
  .getElementById("mi-zoom-out")
  .addEventListener("click", () => setZoom(STATE.zoom - 10));
document
  .getElementById("mi-zoom-reset")
  .addEventListener("click", () => setZoom(100));
// View menu new items
document.getElementById("mi-always-toolbar").addEventListener("click", () => {
  const chk = document.getElementById("mi-always-toolbar-chk");
  const on = chk.style.opacity !== "0";
  chk.style.opacity = on ? "0" : "1";
  DB.saveSetting("alwaysToolbar", !on);
  showToast((on ? "Hide" : "Show") + " toolbar in full screen");
});
document
  .getElementById("mi-customise-toolbar")
  .addEventListener("click", () => {
    openCustomToolbar();
  });
document.getElementById("mi-always-tab-bar").addEventListener("click", () => {
  const chk = document.getElementById("mi-always-tab-bar-chk");
  const on = chk.style.opacity !== "0";
  chk.style.opacity = on ? "0" : "1";
  const tb = document.getElementById("tabBar");
  tb.style.display = on ? "none" : "flex";
  showToast((on ? "Hide" : "Always show") + " Tab Bar");
});
document.getElementById("mi-show-favbar").addEventListener("click", () => {
  ...
  if (!on) {
    if (!fb) {
      ...
      fb.id = "favBar";
      fb.style.cssText =
        "height:28px;background:var(--bg2);border-bottom:1px solid var(--border);display:flex;align-items:center;padding:0 8px;gap:4px;flex-shrink:0;font-size:11px;overflow-x:auto";
      const bms = DB.getBookmarks().slice(0, 8);
      if (bms.length === 0) {
        fb.innerHTML =
          '<span style="color:var(--text3);padding:0 8px">Add bookmarks to see them here</span>';
      } else {
        bms.forEach((b) => {
          const btn = document.createElement("button");
          btn.style.cssText =
            "background:var(--bg3);border:1px solid var(--border2);border-radius:10px;color:var(--text2);padding:2px 10px;cursor:pointer;font-size:11px;white-space:nowrap";
      ...
      navBar.parentNode.insertBefore(fb, navBar.nextSibling);
    }
    fb.style.display = "flex";
    showToast("⭐ Favourites Bar shown");
  } else {
    if (fb) fb.style.display = "none";
    showToast("⭐ Favourites Bar hidden");
  }
});
document.getElementById("mi-show-statusbar").addEventListener("click", () => {
  const chk = document.getElementById("mi-show-statusbar-chk");
  const on = chk.style.opacity !== "0";
  chk.style.opacity = on ? "0" : "1";
  const sb = document.querySelector(".status-bar");
  sb.style.display = on ? "none" : "flex";
  showToast((on ? "Hide" : "Show") + " Status Bar");
});
document.getElementById("mi-show-sidebar").addEventListener("click", () => {
  renderBookmarksPanel();
  togglePanel("bmPanel");
  document.getElementById("mi-show-sidebar-chk").style.opacity = document
    .getElementById("bmPanel")
    .classList.contains("open")
    ? "1"
    : "0";
});
document.getElementById("mi-show-bm-sidebar").addEventListener("click", () => {
  renderBookmarksPanel();
  togglePanel("bmPanel");
  document.getElementById("mi-show-bm-sidebar-chk").style.opacity = document
    .getElementById("bmPanel")
    .classList.contains("open")
    ? "1"
    : "0";
});
document.getElementById("mi-show-rl-sidebar").addEventListener("click", () => {
  const chk = document.getElementById("mi-show-rl-sidebar-chk");
  const rl = RL.get();
  if (!rl.length) {
  ...
  renderHistoryPanel();
  togglePanel("histPanel");
  chk.style.opacity = document
    .getElementById("histPanel")
    .classList.contains("open")
    ? "1"
    : "0";
  showToast("∞ Reading List (" + rl.length + " items)");
});
document
  .getElementById("mi-show-shared")
  .addEventListener("click", () =>
    showToast("👥 Shared with You — not available in web mode"),
  );
document
  .getElementById("mi-show-downloads-view")
  .addEventListener("click", () => togglePanel("dlPanel"));
...
document.getElementById("mi-translation").addEventListener("click", () => {
  const t = getActiveTab();
  if (!t?.url) {
    showToast("No page to translate");
    return;
  }
  const lang = prompt("Translate to (e.g. hr, de, fr, es):", "hr");
  if (lang) {
    navigateTo(
...
document.getElementById("mi-text-encoding").addEventListener("click", () => {
  const enc = prompt("Text encoding (e.g. UTF-8, ISO-8859-1):", "UTF-8");
  if (enc) showToast("⌘ Encoding set to: " + enc);
});
document.getElementById("mi-show-history").addEventListener("click", () => {
  renderHistoryPanel();
  togglePanel("histPanel");
});
document
  .getElementById("mi-back-menu")
  .addEventListener("click", () => document.getElementById("btnBack").click());
document
  .getElementById("mi-fwd-menu")
  .addEventListener("click", () => document.getElementById("btnFwd").click());
document.getElementById("mi-clear-history").addEventListener("click", () => {
  DB.clearHistory();
  showToast("History cleared");
});
document.getElementById("mi-add-bm").addEventListener("click", addBookmark);
document.getElementById("mi-show-bm").addEventListener("click", () => {
  renderBookmarksPanel();
  togglePanel("bmPanel");
});
document.getElementById("mi-start-page").addEventListener("click", () => {
  showNTP();
  const t = getActiveTab();
  if (t) {
    t.url = "";
...
document.getElementById("mi-edit-bm").addEventListener("click", () => {
  renderBookmarksPanel();
  togglePanel("bmPanel");
  showToast("✏️ Edit mode — click × to remove bookmarks");
});
document.getElementById("mi-add-bm-all-tabs").addEventListener("click", () => {
  const count = STATE.tabs.filter((t) => t.url).length;
  if (count === 0) {
    showToast("No pages to bookmark");
    return;
  }
  if (confirm("Add bookmarks for all " + count + " open tab(s)?")) {
    STATE.tabs.forEach((t) => {
      if (t.url) DB.addBookmark({ url: t.url, title: t.title });
    });
...
document.getElementById("mi-add-bm-folder").addEventListener("click", () => {
  ...
  if (name) {
    const folders = JSON.parse(localStorage.getItem("ex_bm_folders") || "[]");
    folders.push({ name, ts: Date.now() });
    localStorage.setItem("ex_bm_folders", JSON.stringify(folders));
...
const RL = {
  ...
  add(e) {
    const l = this.get();
    if (l.find((x) => x.url === e.url)) {
      showToast("Already in Reading List");
      return;
    }
    l.unshift({ ...e, ts: Date.now(), read: false });
    localStorage.setItem("ex_rl", JSON.stringify(l));
    showToast("∞ Added to Reading List: " + e.title);
  },
  idx: 0,
};
document.getElementById("mi-reading-list-add").addEventListener("click", () => {
  const t = getActiveTab();
  if (!t?.url) {
    showToast("No page loaded");
    return;
  }
  RL.add({ url: t.url, title: t.title });
});
document.getElementById("mi-reading-list-all").addEventListener("click", () => {
  const tabs = STATE.tabs.filter((t) => t.url);
  if (!tabs.length) {
    showToast("No pages open");
    return;
  }
  tabs.forEach((t) => RL.add({ url: t.url, title: t.title }));
  showToast("∞ Added " + tabs.length + " tabs to Reading List");
  document.getElementById("mi-reading-list-prev").style.opacity = "1";
  document.getElementById("mi-reading-list-prev").style.cursor = "pointer";
  document.getElementById("mi-reading-list-next").style.opacity = "1";
  document.getElementById("mi-reading-list-next").style.cursor = "pointer";
});
document
  .getElementById("mi-reading-list-prev")
  .addEventListener("click", () => {
    const l = RL.get();
    if (!l.length) return;
    RL.idx = Math.max(0, RL.idx - 1);
    navigateTo(l[RL.idx].url);
  });
document
  .getElementById("mi-reading-list-next")
  .addEventListener("click", () => {
    const l = RL.get();
    if (!l.length) return;
    RL.idx = Math.min(l.length - 1, RL.idx + 1);
    navigateTo(l[RL.idx].url);
  });
document
  .getElementById("mi-devtools")
  .addEventListener("click", toggleDevtools);
document.getElementById("mi-console").addEventListener("click", () => {
  document.querySelector('[data-pane="console"]').click();
});
document
  .getElementById("mi-network")
  .addEventListener("click", () =>
    document.querySelector('[data-pane="network"]').click(),
  );
...
(function initQrSync() {
  ...
  document.querySelectorAll(".qrs-tab").forEach((tab) => {
    tab.addEventListener("click", () => {
      document
        .querySelectorAll(".qrs-tab")
        .forEach((t) => t.classList.remove("active"));
      document
        .querySelectorAll(".qrs-pane")
        .forEach((p) => p.classList.remove("active"));
      tab.classList.add("active");
      document
        .getElementById("qrsp-" + tab.dataset.qrs)
  ...
  function buildPayload() {
    ...
    if (document.getElementById("qrs-bm")?.checked)
      payload.bookmarks = DB.getBookmarks();
    if (document.getElementById("qrs-hist")?.checked)
      payload.history = DB.getHistory().slice(0, 200);
    if (document.getElementById("qrs-ext")?.checked)
      payload.extensions = EXT_DB.get();
    ...
    if (document.getElementById("qrs-profiles")?.checked) {
      payload.profiles = JSON.parse(
        localStorage.getItem("ex_profiles") || "[]",
      );
    }
    return payload;
  }
  ...
  document.getElementById("qrsGenerate").addEventListener("click", () => {
    ...
    const qrUrl =
      ...
      encodeURIComponent(b64.slice(0, 1800)) +
      "&choe=UTF-8";
    img.src = qrUrl;
    img.onerror = () => {
      ...
      note.innerHTML =
        '<strong>Scan not available — copy code below:</strong><br><textarea style="width:100%;font-size:9px;margin-top:6px;background:var(--bg2);border:1px solid var(--border2);border-radius:6px;color:var(--text2);padding:4px;height:80px;resize:none" readonly>' +
        escHtml(b64) +
        "</textarea>";
    };

    const items = Object.keys(payload).filter(
      (k) => !["type", "v", "ts"].includes(k),
    );
    const counts = items
      .map((k) => {
        const v = payload[k];
    ...
    note.innerHTML =
      "<strong>Syncing: " +
      escHtml(counts) +
      '</strong><br>Scan with another EtherX Browser device.<br>Or <a href="#" id="qrsCopyCode" style="color:var(--accent)">copy sync code</a> to paste manually.';
  ...
  document.getElementById("qrsImport").addEventListener("click", () => {
    ...
    if (!raw) {
      showToast("⚠️ Paste a sync code first");
      return;
    }
    try {
      const json = decodeURIComponent(escape(atob(raw)));
      const data = JSON.parse(json);
      if (data.type !== "etherx-sync") {
        showToast("❌ Invalid sync code");
        return;
      }
      let imported = [];
      if (data.bookmarks) {
        data.bookmarks.forEach((b) => DB.addBookmark(b));
        imported.push(data.bookmarks.length + " bookmarks");
      }
      if (data.extensions) {
        data.extensions.forEach((e) => EXT_DB.add(e));
        imported.push(data.extensions.length + " extensions");
        renderExtIconBar();
      }
      if (data.settings) {
        Object.entries(data.settings).forEach(([k, v]) => DB.saveSetting(k, v));
        imported.push("settings");
      }
      if (data.profiles) {
        localStorage.setItem("ex_profiles", JSON.stringify(data.profiles));
        imported.push(data.profiles.length + " profiles");
      }
      renderBookmarksPanel();
      renderBmFolderBar();
      document.getElementById("qrsImportResult").innerHTML =
...
document
  .getElementById("mi-inspect-devices")
  .addEventListener("click", () =>
    showToast("📱 No physical devices connected"),
  );
document.getElementById("mi-service-workers").addEventListener("click", () => {
  toggleDevtools();
  consoleLog("info", "⚙️ Service Workers: none registered on proxied pages");
});
document
  .getElementById("mi-web-ext-bg")
  .addEventListener("click", () =>
    showToast("🧩 No extension background content"),
  );
document
  .getElementById("mi-web-inspector")
  .addEventListener("click", () =>
    showToast("🔗 Web Inspector: connect via Safari on device"),
  );
document.getElementById("mi-page-source").addEventListener("click", () => {
  ...
  if (codeEl)
    codeEl.innerHTML =
      '<span style="color:#858585;font-size:11px">⏳ Loading source for ' +
      escHtml(t.url) +
      "…</span>";
  // Add to sources tree
  if (treeEl) {
    const existing = treeEl.querySelector(
      '[data-src-url="' + CSS.escape(t.url) + '"]',
    );
    if (!existing) {
      const item = document.createElement("div");
      item.className = "src-item";
    ...
    document
      ...
      .forEach((x) => x.classList.remove("active"));
    const target =
      treeEl.querySelector('[data-src-url="' + CSS.escape(t.url) + '"]') ||
      existing;
...
function loadSourceIntoPane(url, codeEl) {
      ...
      safeWebviewExecute(
        ...
        .then((src) => {
          const esc = src
            ...
            .replace(/>/g, "&gt;");
          codeEl.innerHTML =
            '<pre style="white-space:pre-wrap;color:#d4d4d4;font-size:12px;padding:12px">' +
            esc +
            "</pre>";
        })
        .catch(() => {
          codeEl.innerHTML =
            '<div style="color:#e06c75;padding:12px">Source unavailable in Electron mode</div>';
        });
    } else {
      codeEl.innerHTML =
        '<div style="color:#e06c75;padding:12px">Source unavailable</div>';
    }
    return;
  }
  ...
  fetch(url)
    ...
    .then((src) => {
      ...
      codeEl.innerHTML =
        '<div style="counter-reset:ln;line-height:1.6">' +
        highlighted
          .split("\n")
          .map(
            (line, i) =>
    ...
    .catch(() => {
      codeEl.innerHTML =
        '<span style="color:#f48771">Source not available (blocked by CORS).</span>';
    });
}
document.getElementById("mi-page-resources").addEventListener("click", () => {
  document.querySelector('[data-pane="network"]').click();
  showToast("📦 Page Resources — see Network tab");
});
...
document.getElementById("mi-timeline").addEventListener("click", () => {
  ...
  showToast(
    timelineRunning
      ? "⏱️ Timeline recording started…"
      : "⏹ Timeline recording stopped",
  );
  if (timelineRunning) {
    toggleDevtools();
    consoleLog("info", "⏱️ Timeline recording started");
  } else {
    consoleLog("warn", "⏹ Timeline recording stopped");
  }
});
...
document.getElementById("mi-elem-select").addEventListener("click", () => {
  ...
  if (elemSelecting) {
    const handler = (e) => {
      ...
      document
        .getElementById("mi-elem-select")
        .querySelector("span:not(.icon)").textContent =
        "Start Element Selection";
      toggleDevtools();
      document.querySelector('[data-pane="elements"]').click();
      consoleLog(
        "info",
        "🔲 Selected: &lt;" +
...
document.getElementById("mi-clear-caches").addEventListener("click", () => {
  if (confirm("Empty all caches for all profiles?")) {
    DB.clearHistory();
    localStorage.clear();
    showToast("🗑️ Caches cleared for all profiles");
    consoleLog("warn", "🗑️ All caches cleared");
  }
});
...
document.getElementById("mi-mute-others").addEventListener("click", () => {
  ...
  showToast("🔈 Other tabs muted");
});
document.getElementById("mi-dup-tab").addEventListener("click", dupTab);
document.getElementById("mi-prev-tab").addEventListener("click", () => {
  const i = STATE.tabs.findIndex((t) => t.id === STATE.activeTabId);
  if (i > 0) switchTab(STATE.tabs[i - 1].id);
});
document.getElementById("mi-next-tab").addEventListener("click", () => {
  const i = STATE.tabs.findIndex((t) => t.id === STATE.activeTabId);
  if (i < STATE.tabs.length - 1) switchTab(STATE.tabs[i + 1].id);
});
document
  .getElementById("mi-downloads")
  .addEventListener("click", () => togglePanel("dlPanel"));
document
  .getElementById("mi-minimise")
  .addEventListener("click", () =>
    showToast("▭ Minimise — not available in web mode"),
  );
...
document
  .getElementById("mi-win-centre")
  .addEventListener("click", () =>
    showToast("◉ Centre — not available in web mode"),
  );
document
  .getElementById("mi-move-resize")
  .addEventListener("click", () =>
    showToast("⤡ Move & Resize — use browser window controls"),
  );
document
  .getElementById("mi-fullscreen-tile")
  .addEventListener("click", () =>
    showToast("⊞ Full-Screen Tile — not available in web mode"),
  );
document.getElementById("mi-arrange-tabs").addEventListener("click", () => {
  STATE.tabs.sort((a, b) => a.title.localeCompare(b.title));
  document.querySelectorAll(".tab").forEach((el) => el.remove());
  const nb = document.getElementById("newTabBtn");
  STATE.tabs.forEach((t) => {
    renderTab(t);
  });
  switchTab(STATE.activeTabId);
  showToast("⇅ Tabs arranged by title");
});
...
document.getElementById("mi-move-tab-window").addEventListener("click", () => {
  const t = getActiveTab();
  if (t?.url) {
    window.open("/browser.html#" + encodeURIComponent(t.url), "_blank");
    closeTab(t.id);
  } else showToast("No page to move");
});
document.getElementById("mi-bring-front").addEventListener("click", () => {
  window.focus();
  showToast("⬆ Brought to front");
});
document.getElementById("mi-open-location").addEventListener("click", () => {
  urlInput.focus();
  urlInput.select();
});
document.getElementById("mi-print").addEventListener("click", () => {
  try {
    ...
    showToast("Cannot print cross-origin page");
  }
});
document
  .getElementById("mi-reader")
  .addEventListener("click", () =>
...
document
  .getElementById("togglePageDarkMode")
...
document
  .getElementById("mi-about")
  .addEventListener("click", () =>
    showToast("EtherX Browser v1.0 — Web3-Native Browser"),
  );
document.getElementById("mi-shortcuts").addEventListener("click", () => {
  alert(
...
function pinTab() {
  ...
  t.pinned = !t.pinned;
  updateTabEl(t);
  showToast(t.pinned ? "📌 Tab pinned" : "📌 Tab unpinned");
}
function muteTab() {
  const t = getActiveTab();
  if (!t) return;
...
function addBookmark() {
  ...
  DB.addBookmark({ url: t.url, title: t.title });
}
document
  .getElementById("btnBookmarkPage")
  ?.addEventListener("click", addBookmark);
const DEFAULT_QUICKLINKS = [
  { url: "https://en.wikipedia.org", label: "Wiki", icon: "📖" },
  { url: "https://github.com", label: "GitHub", icon: "��" },
  { url: "https://app.uniswap.org", label: "Uniswap", icon: "🦄" },
  { url: "https://etherscan.io", label: "Etherscan", icon: "🔍" },
];
function renderQuickLinks() {
  ...
  if (hist.length > 0) {
    links = hist.map((h) => ({
      url: h.url,
      label: (h.title || h.url).slice(0, 12),
      icon: getSiteFavicon(h.url),
    }));
  } else {
  ...
  links.forEach((l) => {
    ...
    el.innerHTML = `<div class="ql-icon">${l.icon}</div><div class="ql-label">${l.label}</div>`;
    el.addEventListener("click", () => navigateTo(l.url));
    ql.appendChild(el);
  });
}
...
function renderRecentSites() {
  ...
  r.forEach((s) => {
    const el = document.createElement("div");
    el.className = "ntp-recent-card";
    let host = "";
    try {
      host = new URL(s.url).hostname;
    } catch (e) {
      host = s.url.slice(0, 20);
    }
    ...
    el.addEventListener("click", (e) => {
      if (e.target.classList.contains("nrc-del")) return;
      navigateTo(s.url);
    });
    el.querySelector(".nrc-del").addEventListener("click", (e) => {
      e.stopPropagation();
      let rx = JSON.parse(localStorage.getItem("ex_recent") || "[]");
      rx = rx.filter((x) => x.url !== s.url);
      localStorage.setItem("ex_recent", JSON.stringify(rx));
      renderRecentSites();
    });
    cont.appendChild(el);
  });
...
function applyNtpSettings() {
  ...
  const ntp = document.getElementById("ntpPage");
  if (s.bgUrl) {
    ntp.style.backgroundImage = "url('" + s.bgUrl + "')";
    ntp.style.backgroundSize = "cover";
    ntp.style.backgroundPosition = "center";
  } else if (s.bg) {
    ntp.style.background = s.bg;
    ntp.style.backgroundImage = "";
  }
  if (s.title) document.getElementById("ntpTitle").textContent = s.title;
  if (s.subtitle)
    document.getElementById("ntpSubtitle").textContent = s.subtitle;
  const showRecent = s.showRecent !== false;
  const showCards = s.showCards !== false;
  const showQL = s.showQL !== false;
  const rs = document.getElementById("ntpRecentSection");
  if (rs) rs.style.display = showRecent ? "" : "none";
  const nc = document.querySelector(".ntp-cards");
  if (nc) nc.style.display = showCards ? "" : "none";
  const ql = document.getElementById("quickLinks");
  if (ql) ql.style.display = showQL ? "" : "none";
}
document.addEventListener("DOMContentLoaded", () => {
  const ntpEditBtn = document.getElementById("ntpEditBtn");
  if (ntpEditBtn)
    ntpEditBtn.addEventListener("click", () => {
      const s = JSON.parse(localStorage.getItem("ex_ntp") || "{}");
      document.getElementById("ntpcBgUrl").value = s.bgUrl || "";
      document.getElementById("ntpcTitle").value = s.title || "EtherX Browser";
      document.getElementById("ntpcSubtitle").value =
        s.subtitle || "The Web3-Native Browser Experience";
      document
        .getElementById("ntpcShowRecent")
        .classList.toggle("on", s.showRecent !== false);
      document
        .getElementById("ntpcShowCards")
        .classList.toggle("on", s.showCards !== false);
      document
        .getElementById("ntpcShowQL")
        .classList.toggle("on", s.showQL !== false);
      document.getElementById("ntpCustomize").classList.add("show");
    });
  const ntpcClose = document.getElementById("ntpCustomizeClose");
  if (ntpcClose)
    ntpcClose.addEventListener("click", () =>
      document.getElementById("ntpCustomize").classList.remove("show"),
    );
  document.querySelectorAll(".ntpc-color").forEach((c) =>
    c.addEventListener("click", () => {
      document
        .querySelectorAll(".ntpc-color")
  ...
  if (ntpcApply)
    ntpcApply.addEventListener("click", () => {
      const s = {};
      const bgUrl = document.getElementById("ntpcBgUrl").value.trim();
      if (bgUrl) s.bgUrl = bgUrl;
      else {
        const selCol = document.querySelector(".ntpc-color.sel");
        if (selCol) s.bg = selCol.dataset.bg;
        else {
          const cc = document.getElementById("ntpcCustomColor").value;
          if (cc) s.bg = cc;
        }
      }
      ...
      s.subtitle =
        document.getElementById("ntpcSubtitle").value ||
        "The Web3-Native Browser Experience";
      s.showRecent = document
        .getElementById("ntpcShowRecent")
        .classList.contains("on");
      s.showCards = document
        .getElementById("ntpcShowCards")
        .classList.contains("on");
...
{
  const dtR = document.getElementById("dtResize");
  let dtDrag = false,
    dtSY = 0,
    dtSH = 0,
    dtSX = 0,
    dtSW = 0;
  dtR.addEventListener("mousedown", (e) => {
    const dt = document.getElementById("devtools");
    dtDrag = true;
    if (
      dt.classList.contains("dock-left") ||
      dt.classList.contains("dock-right")
    ) {
      dtSX = e.clientX;
      dtSW = dt.offsetWidth;
    } else {
      dtSY = e.clientY;
      dtSH = dt.offsetHeight;
    }
    e.preventDefault();
  });
  document.addEventListener("mousemove", (e) => {
    if (!dtDrag) return;
    const dt = document.getElementById("devtools");
    if (dt.classList.contains("dock-left")) {
      dt.style.width =
        Math.max(
          280,
          Math.min(window.innerWidth - 100, dtSW + (e.clientX - dtSX)),
        ) + "px";
    } else if (dt.classList.contains("dock-right")) {
      dt.style.width =
        Math.max(
          280,
          Math.min(window.innerWidth - 100, dtSW - (e.clientX - dtSX)),
      ...
      dt.style.height =
        Math.max(
          120,
          Math.min(window.innerHeight - 160, dtSH + (dtSY - e.clientY)),
        ) + "px";
    }
  });
  document.addEventListener("mouseup", () => {
...
function collectMemoryMetrics() {
  ...
  if (performance.memory) {
    used = fmt(performance.memory.usedJSHeapSize);
    total = fmt(performance.memory.totalJSHeapSize);
    limit = fmt(performance.memory.jsHeapSizeLimit);
  ...
  document.getElementById("memNodes").textContent =
    document.querySelectorAll("*").length;
  document.getElementById("memListeners").textContent =
    "~" + Math.floor(Math.random() * 80 + 20);
  document.getElementById("memObjects").textContent =
...
function renderSecurityPanel() {
  ...
  function badge(id, cls, txt) {
    const el = document.getElementById(id);
    if (el) {
      el.className = "sec-badge " + cls;
      el.textContent = txt;
    }
  }
  if (!url) {
    set("secConnection", "No page loaded");
    [
      "secConnBadge",
      "secCertBadge",
      "secCspBadge",
  ...
  set(
    ...
    isHttps
      ? "HTTPS — " + url.split("/")[2]
      : "HTTP (insecure) — " + url.split("/")[2],
  );
  badge(
    "secConnBadge",
    isHttps ? "sec-ok" : "sec-bad",
    isHttps ? "Secure" : "Not Secure",
  );
  set(
    "secCert",
    isHttps ? "TLS 1.3, valid certificate" : "No certificate (HTTP)",
  );
  badge(
    "secCertBadge",
    isHttps ? "sec-ok" : "sec-bad",
  ...
  badge("secCookBadge", ck ? "sec-warn" : "sec-ok", ck ? "Present" : "None");
  set("secMixed", isHttps ? "No mixed content detected" : "HTTP page");
  badge(
    "secMixedBadge",
    isHttps ? "sec-ok" : "sec-warn",
    isHttps ? "None" : "N/A",
  );
  set("secHsts", isHttps ? "HSTS active" : "Not applicable");
  badge(
    "secHstsBadge",
    isHttps ? "sec-ok" : "sec-warn",
    isHttps ? "Active" : "None",
  ...
  if (cachedCsp !== undefined) {
    if (cachedCsp) {
      set(
        "secCsp",
        cachedCsp.slice(0, 120) + (cachedCsp.length > 120 ? "…" : ""),
      );
      badge("secCspBadge", "sec-ok", "Present");
    } else {
      set("secCsp", "No CSP header found");
      badge("secCspBadge", "sec-warn", "Missing");
    }
  } else if (!window.electronWebview) {
    set("secCsp", "Fetching…");
    badge("secCspBadge", "sec-warn", "…");
    // Standalone: CSP headers not accessible via CORS fetch
    set("secCsp", "Not available in standalone mode");
    badge("secCspBadge", "sec-warn", "N/A");
  } else {
    set("secCsp", "Not accessible in Electron mode");
    badge("secCspBadge", "sec-warn", "Unknown");
  }
}
...
document.getElementById("lhRun").addEventListener("click", () => {
  ...
  document.getElementById("lhStatus").textContent = "Running audit...";
  document.getElementById("lhRun").disabled = true;
  setTimeout(() => {
    ...
    [
      ...
      const el = document.getElementById(id);
      el.className = "lh-circle " + cl(v);
      el.textContent = v;
    });
    const audits = [
      {
        c: sc.perf >= 90 ? "g" : "o",
        t: "First Contentful Paint",
        v: r([0, 100]) + "ms",
      },
      {
        c: sc.perf >= 75 ? "g" : "o",
      ...
      },
      {
        c: sc.a11y >= 90 ? "g" : "o",
        t: "Image alt text",
        v: sc.a11y >= 90 ? "All present" : "Some missing",
      },
      {
        c: sc.seo >= 90 ? "g" : "r",
        t: "Meta description",
        v: sc.seo >= 90 ? "Present" : "Missing",
      },
      {
        c: "g",
        t: "HTTPS",
        v: t.url.startsWith("https") ? "Secure" : "Not secure",
      },
      {
        c: sc.pwa >= 50 ? "g" : "r",
        t: "Web App Manifest",
        v: sc.pwa >= 50 ? "Found" : "Missing",
      },
    ];
    document.getElementById("lhAudits").innerHTML =
      '<div style="font-size:10px;color:#858585;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">Audit Results</div>' +
      audits
        .map(
          (a) =>
            `<div class="lh-item"><div class="lh-dot ${a.c}"></div><span style="flex:1;font-size:11px;color:#ccc">${escHtml(a.t)}</span><span style="font-size:10px;color:#858585">${escHtml(a.v)}</span></div>`,
        )
        .join("");
    document.getElementById("lhStatus").textContent =
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".ua-preset").forEach((btn) => {
    btn.addEventListener("click", () => {
      document
        .querySelectorAll(".ua-preset")
        .forEach((b) => b.classList.remove("active"));
      btn.classList.add("active");
      const ua = UA_PRESETS[btn.dataset.ua] || navigator.userAgent;
      document.getElementById("uaCustomInput").value = ua;
      document.getElementById("uaCurrent").textContent = ua;
    });
  });
  const uaApply = document.getElementById("uaApply");
  if (uaApply)
    uaApply.addEventListener("click", () => {
      const ua =
        document.getElementById("uaCustomInput").value.trim() ||
        navigator.userAgent;
      activeUA = ua;
      document.getElementById("uaCurrent").textContent = ua;
      localStorage.setItem("ex_ua", ua);
      showToast("\u2713 User Agent: " + ua.slice(0, 40) + "...");
      consoleLog("info", "\uD83D\uDD75\uFE0F UA changed: " + ua.slice(0, 60));
    });
});

// Extensions DB + Manager
const EXT_DB = {
  get: () => JSON.parse(localStorage.getItem("ex_extensions") || "[]"),
  save(a) {
    localStorage.setItem("ex_extensions", JSON.stringify(a));
  },
  add(ext) {
    const a = this.get();
    if (a.find((e) => e.id === ext.id)) {
...
(function seedDefaultExtensions() {
  const defaults = [
    {
      ...
      icon: "🔑",
      desc: "Use iCloud Keychain passwords in your browser",
      enabled: true,
      source:
  ...
  defaults.forEach((def) => {
    if (!existing.find((e) => e.id === def.id)) {
      existing.push(def);
      changed = true;
    }
  });
...
function renderExtList() {
  ...
  if (!exts.length) {
    list.innerHTML =
      '<div style="padding:24px;text-align:center;color:var(--text3);font-size:12px"><div style="font-size:32px;margin-bottom:10px">🧩</div>No extensions installed.</div>';
    renderExtIconBar();
    updateSettingsExtCount && updateSettingsExtCount();
    return;
  }
  ...
  exts.forEach((ext) => {
    ...
    el.querySelector(".ext-toggle").addEventListener("click", () => {
      EXT_DB.toggle(ext.id);
      renderExtList();
      renderExtIconBar();
    });
    el.querySelector("[data-del]").addEventListener("click", () => {
      window.customConfirm('Remove "' + ext.name + '"?', (c) => {
        if (c) {
          EXT_DB.remove(ext.id);
          renderExtList();
          renderExtIconBar();
...
function renderExtIconBar() {
  const bar = document.getElementById("extIconBar");
  if (!bar) return;
  const exts = EXT_DB.get();
  bar.innerHTML = "";
  exts.forEach((ext) => {
    ...
    pill.innerHTML =
      (ext.icon || "🧩") +
      `<span class="ext-pill-tooltip">${escHtml(ext.name)}${ext.enabled ? "" : " ⏸"}</span>`;
    pill.addEventListener("click", () => {
      const url =
        ext.source || "https://chromewebstore.google.com/detail/" + ext.id;
      closeAllPanels();
      navigateTo(url);
...
document.addEventListener("keydown", (e) => {
  if (
    e.key === "Escape" &&
    document.getElementById("iconPickerOverlay").classList.contains("show")
  ) {
    document.getElementById("iconPickerOverlay").classList.remove("show");
    _iconPickerCallback = null;
  }
...
document
  .querySelectorAll(".s-icon-edit-btn[data-icon-target]")
  .forEach((editBtn) => {
    editBtn.addEventListener("click", () => {
      ...
      openIconPicker(
        "Promijeni ikonu: " + targetId,
        current,
        TOOLBAR_ICON_PRESETS,
        (icon) => {
...
document
  .querySelectorAll(".s-icon-edit-btn[data-icon-target-text]")
  .forEach((editBtn) => {
    editBtn.addEventListener("click", () => {
  ...
  Object.keys(s).forEach((k) => {
    if (k.startsWith("customIcon_")) {
      ...
      const editBtn = document.querySelector(
        '.s-icon-edit-btn[data-icon-target="' + id + '"]',
      );
    ...
    if (k.startsWith("titlebarIcon_")) {
      ...
      const editBtn = document.querySelector(
        '.s-icon-edit-btn[data-icon-target-text="' + id + '"]',
      );
      if (editBtn) editBtn.textContent = icon;
      const el = document.getElementById(id);
      if (el) {
...
function renderBmFolderBar() {
  const inner = document.getElementById("bmFolderBarInner");
  if (!inner) return;
  const bms = DB.getBookmarks();
  inner.innerHTML = "";
  if (!bms.length) {
    inner.innerHTML =
      '<span style="color:var(--text3);font-size:11px;padding:0 8px">No bookmarks — press Ctrl+D to save one</span>';
    return;
  }
  // Build folder groups
  const folders = {};
...
DB.addBookmark = function (e) {
  _origAddBm(e);
  renderBmFolderBar();
};
const _origRemBm = DB.removeBookmark.bind(DB);
DB.removeBookmark = function (u) {
  _origRemBm(u);
  renderBmFolderBar();
};
...
afBox.style.cssText =
  "position:fixed;background:var(--bg2);border:1px solid var(--border2);border-radius:12px;box-shadow:0 8px 32px rgba(0,0,0,.4);z-index:9999;min-width:260px;max-width:320px;display:none;flex-direction:column;overflow:hidden";
...
function showAutofillBox(x, y, items, onSelect) {
  ...
  hdr.style.cssText =
    "padding:8px 12px 6px;font-size:10px;color:var(--text3);font-weight:600;text-transform:uppercase;letter-spacing:.5px;border-bottom:1px solid var(--border)";
  hdr.textContent =
    items[0]._type === "password" ? "🔑 Saved Passwords" : "💳 Saved Cards";
  afBox.appendChild(hdr);
  items.forEach((item) => {
    const row = document.createElement("div");
    row.style.cssText =
      "padding:9px 12px;cursor:pointer;display:flex;flex-direction:column;gap:2px;transition:background .1s";
    row.addEventListener(
      "mouseenter",
      () => (row.style.background = "var(--border2)"),
    );
    ...
    if (item._type === "password") {
      row.innerHTML =
        ...
        '</span><span style="font-size:10px;color:var(--text3)">' +
        escHtml(item.site) +
        "</span>";
    } else {
      row.innerHTML =
  ...
  afBox.style.left = bx + "px";
  afBox.style.top = by + "px";
}
document.addEventListener("click", (e) => {
  if (!afBox.contains(e.target)) hideAutofillBox();
});
...
(function addAutofillBtn() {
  ...
  btn.innerHTML = "🔑";
  btn.style.cssText =
    "background:none;border:none;cursor:pointer;font-size:14px;padding:2px 4px;opacity:0.6;transition:opacity .15s;flex-shrink:0";
      ...
      .filter(
        (p) =>
          ...
          p.site
            ...
            .replace("www.", "")
            .startsWith(host) ||
          host.includes(p.site.replace(/https?:\/\//, "").replace("www.", "")),
      )
      .map((p) => ({ ...p, _type: "password" }));
    const cards = getCards().map((c) => ({ ...c, _type: "card" }));
    const all = [...pwds, ...cards];
    if (!all.length) {
      showToast("🔑 No saved passwords or cards for this site");
      return;
    }
...
(function initCardsSettings() {
  ...
  function renderCardList() {
    ...
    if (!cards.length) {
      cardList.innerHTML =
        '<div style="color:var(--text3);font-size:11px;padding:4px 0">No saved cards</div>';
      return;
    }
    cards.forEach((c, i) => {
      const row = document.createElement("div");
      row.style.cssText =
        "display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--border);font-size:11px";
      row.innerHTML =
        '<span style="flex:1">💳 ' +
        escHtml(c.holder) +
        " •••• " +
  ...
  addCardBtn?.addEventListener("click", () => {
    const holder = prompt("Cardholder name:");
    if (!holder) return;
    const number = prompt("Card number (16 digits):");
    if (!number) return;
...
document.getElementById("extAddBtn").addEventListener("click", () => {
  const val = document.getElementById("extAddUrl").value.trim();
  if (!val) return;
  let id = val,
    name = val,
    icon = "\uD83E\uDDE9",
    desc = "Installed from Chrome Web Store";
  const m = val.match(/[a-z]{32}/i);
  if (m) id = m[0];
  ...
  const known = {
    ...
    "iCloud Passwords": "pejdijmoenmkgeppbflobdenhhabjlaj",
  };
  const icons = {
    MetaMask: "\uD83E\uDD8A",
    "uBlock Origin": "\uD83D\uDEE1\uFE0F",
  ...
  for (const [n, kid] of Object.entries(known)) {
    if (val.includes(kid) || val.toLowerCase().includes(n.toLowerCase())) {
      name = n;
      id = kid;
      icon = icons[n] || "\uD83E\uDDE9";
    }
  ...
  renderExtList();
  document.getElementById("extAddUrl").value = "";
  showToast("\uD83E\uDDE9 Installed: " + name);
  consoleLog("success", "\uD83E\uDDE9 Extension installed: " + name);
});
document.getElementById("extAddUrl").addEventListener("keydown", (e) => {
  if (e.key === "Enter") document.getElementById("extAddBtn").click();
});
document.getElementById("btnExtensions").addEventListener("click", () => {
  ...
  closeAllPanels();
  ep.classList.toggle("open", !isOpen);
  if (!isOpen) renderExtList();
});
document
  .getElementById("closeExtPanel")
  .addEventListener("click", () =>
...
document.querySelectorAll(".ext-tab").forEach((tab) => {
  tab.addEventListener("click", () => {
    document
      ...
      .forEach((t) => t.classList.remove("active"));
    document
      .querySelectorAll(".ext-pane")
      .forEach((p) => p.classList.remove("active"));
    tab.classList.add("active");
    const pane = document.getElementById("extpane-" + tab.dataset.extPane);
    if (pane) pane.classList.add("active");
    if (tab.dataset.extPane === "installed") renderExtList();
  });
});
...
document.getElementById("btnOpenCWS")?.addEventListener("click", () => {
  ...
  if (typeof require !== "undefined") {
    try {
      require("electron").shell.openExternal(cwsUrl);
      return;
    } catch (e) {
...
document.querySelectorAll(".ext-install-btn").forEach((btn) => {
  btn.addEventListener("click", function () {
    ...
    document
      ...
      .forEach((p) => p.classList.remove("active"));
    document
      .querySelector('[data-ext-pane="installed"]')
      ?.classList.add("active");
...
frame.addEventListener(
  ...
  () => {
    ...
    if (
      ...
      return;
    const t = getActiveTab();
    if (t && t.url) addRecentSite(t.url, t.title);
  },
...
document
  .querySelectorAll(".ntp-card")
  .forEach((card) =>
    card.addEventListener("click", () => navigateTo(card.dataset.url)),
  );
document.getElementById("boOpen").addEventListener("click", () => {
  const t = getActiveTab();
  if (t?.url) window.open(t.url, "_blank");
});
document.getElementById("boRetry").addEventListener("click", () => {
  const t = getActiveTab();
  if (t?.url) navigateTo(t.url);
});
...
function showCtxMenu(x, y, targetUrl, imageUrl) {
  ...
  [
    ...
    "ctx-open-tab-group",
    "ctx-download-link",
    "ctx-download-link-as",
    "ctx-reading-list",
  ].forEach((id) => {
    const el = document.getElementById(id);
    if (el) el.style.display = hasLink ? "" : "none";
  });
  document
    .querySelectorAll(".ctx-sep:not(.ctx-no-link), #ctx-copy-url")
    .forEach((el) => {
      el.style.display = "";
    });
  // Show/hide image-specific items
  const hasImage = !!imageUrl;
  document.querySelectorAll(".ctx-img-item").forEach((el) => {
    el.style.display = hasImage ? "" : "none";
  });
  document.querySelectorAll(".ctx-img-sep").forEach((el) => {
    el.style.display = hasImage ? "" : "none";
  });
  // Position
  const menuW = 240,
...
document
  .getElementById("ctx-new-tab")
  .addEventListener("click", () => createTab());
document
  .getElementById("ctx-reload")
  .addEventListener("click", () =>
    document.getElementById("btnReload").click(),
  );
document.getElementById("ctx-find").addEventListener("click", openFind);
document
  .getElementById("ctx-devtools")
  .addEventListener("click", toggleDevtools);
document
  .getElementById("ctx-close-tab")
  .addEventListener("click", () => closeTab(_ctxTabId || STATE.activeTabId));
...
document
  .getElementById("ctx-download-link-as")
  .addEventListener("click", () => {
    if (_ctxTargetUrl) {
...
document.getElementById("ctx-bookmark").addEventListener("click", () => {
  ...
  if (url) {
    DB.addBookmark({ url, title, ts: Date.now() });
...
function _revEyeSearch(engineId) {
  ...
  const url = _revEyeEngines[engineId].replace(
    "%s",
    encodeURIComponent(_ctxImageUrl),
  );
  createTab(url);
}
document
  .getElementById("ctx-img-search-google")
  .addEventListener("click", () => _revEyeSearch("google"));
document
  .getElementById("ctx-img-search-bing")
  .addEventListener("click", () => _revEyeSearch("bing"));
document
  .getElementById("ctx-img-search-yandex")
  .addEventListener("click", () => _revEyeSearch("yandex"));
document
  .getElementById("ctx-img-search-tineye")
  .addEventListener("click", () => _revEyeSearch("tineye"));
document.getElementById("ctx-img-copy-url").addEventListener("click", () => {
  if (_ctxImageUrl)
    navigator.clipboard
      .writeText(_ctxImageUrl)
...
document.getElementById("ctx-share").addEventListener("click", () => {
  ...
  if (navigator.share && url) {
    ...
    navigator.clipboard
      .writeText(url)
      .then(() => showToast("📋 Link copied to share"));
  }
});
...
function updateClock() {
  ...
  const d = now.toLocaleDateString("hr-HR", {
    weekday: "short",
    day: "numeric",
    month: "short",
  });
  const clockPfx =
    (typeof _titlebarIcons !== "undefined" && _titlebarIcons["titleClock"]) ||
    "";
  const datePfx =
    (typeof _titlebarIcons !== "undefined" && _titlebarIcons["titleDate"]) ||
    "";
  const clockEl = document.getElementById("titleClock");
  clockEl.textContent = (clockPfx ? clockPfx + " " : "") + t;
  const savedCfg = DB.getSettings();
  if (savedCfg.clockColor) clockEl.style.color = savedCfg.clockColor;
  if (savedCfg.clockSize) clockEl.style.fontSize = savedCfg.clockSize + "px";
  document.getElementById("sbTime").textContent = t;
  const de = document.getElementById("titleDate");
  if (de) de.textContent = (datePfx ? datePfx + " " : "") + d;
}
updateClock();
...
document.addEventListener("keydown", (e) => {
  ...
  if (ctrl && e.key === "t") {
    ...
    e.preventDefault();
    document.getElementById("mi-next-tab").click();
  } else if (ctrl && shift && e.key === "Tab") {
    e.preventDefault();
    document.getElementById("mi-prev-tab").click();
  } else if (e.key === "F12") {
    e.preventDefault();
    toggleDevtools();
  } else if (e.key === "F11") {
    e.preventDefault();
    document.getElementById("mi-fullscreen").click();
  } else if (
    (function () {
      const sc = (DB.getSettings().screenshotShortcut || "Ctrl+Shift+S").split(
        "+",
      );
      const needCtrl = sc.includes("Ctrl"),
        needAlt = sc.includes("Alt"),
        needShift = sc.includes("Shift");
      const mainKey =
        sc.filter((k) => !["Ctrl", "Alt", "Shift"].includes(k))[0] || "";
      return (
        ctrl === needCtrl &&
        e.altKey === needAlt &&
        shift === needShift &&
        (e.key === mainKey || e.key.toUpperCase() === mainKey.toUpperCase())
      );
    })()
  ) {
    e.preventDefault();
    ...
    document.getElementById("tabOverview").classList.remove("show");
...
(function initMigration() {
  ...
  const DEMO_BOOKMARKS = {
    ...
    "Mozilla Firefox": [
      ...
      { url: "https://twitter.com", title: "Twitter / X" },
    ],
    "Microsoft Edge": [
      { url: "https://bing.com", title: "Bing" },
      { url: "https://outlook.com", title: "Outlook" },
      { url: "https://office.com", title: "Microsoft 365" },
      { url: "https://linkedin.com", title: "LinkedIn" },
      { url: "https://onedrive.live.com", title: "OneDrive" },
      { url: "https://teams.microsoft.com", title: "Microsoft Teams" },
      { url: "https://copilot.microsoft.com", title: "Copilot" },
    ],
    Safari: [
    ...
    Brave: [
      ...
      { url: "https://basicattentiontoken.org", title: "BAT" },
      { url: "https://etherscan.io", title: "Etherscan" },
      { url: "https://coinmarketcap.com", title: "CoinMarketCap" },
      { url: "https://uniswap.org", title: "Uniswap" },
      { url: "https://metamask.io", title: "MetaMask" },
    ...
    Opera: [
      { url: "https://opera.com", title: "Opera" },
      { url: "https://addons.opera.com", title: "Opera Addons" },
      { url: "https://blogs.opera.com", title: "Opera Blog" },
      { url: "https://opera.com/gx", title: "Opera GX" },
      { url: "https://opera.com/crypto", title: "Opera Crypto" },
    ],
  };
  const DEMO_HISTORY = {
    "Google Chrome": [
      {
        url: "https://google.com/search?q=web3",
        title: "web3 - Google Search",
      },
      { url: "https://docs.google.com", title: "Google Docs" },
      { url: "https://maps.google.com", title: "Google Maps" },
      { url: "https://github.com/trending", title: "Trending - GitHub" },
      { url: "https://youtube.com/feed/trending", title: "Trending - YouTube" },
    ],
    "Mozilla Firefox": [
      {
    ...
    "Microsoft Edge": [
      { url: "https://bing.com/search?q=crypto", title: "crypto - Search" },
      { url: "https://copilot.microsoft.com", title: "Microsoft Copilot" },
      { url: "https://outlook.com/mail/inbox", title: "Inbox – Outlook" },
    ],
    Safari: [
      ...
      { url: "https://etherscan.io", title: "Etherscan" },
      {
        url: "https://google.com/search?q=ethereum",
        title: "ethereum - Google",
      },
    ...
    Brave: [
      {
        url: "https://search.brave.com/search?q=defi",
        title: "defi - Brave Search",
      },
      { url: "https://etherscan.io/txs", title: "Transactions - Etherscan" },
    ],
    Opera: [
      { url: "https://opera.com/features", title: "Opera Features" },
      { url: "https://opera.com/gx", title: "Opera GX" },
    ],
  };
  document
    .getElementById("btnStartMigration")
    ?.addEventListener("click", () => {
      const source = document.getElementById("migrationSource").value;
      const impBm = document
        .getElementById("migImportBookmarks")
        ?.classList.contains("on");
      const impHist = document
        .getElementById("migImportHistory")
        ?.classList.contains("on");
      const status = document.getElementById("migrationStatus");
      let count = 0;
      status.innerHTML =
        '<span style="color:var(--accent)">Importing from ' +
        source +
        "...</span>";
      setTimeout(() => {
  ...
  document
    .getElementById("migrationFileInput")
        ...
        try {
          ...
          if (file.name.toLowerCase().endsWith(".json")) {
            ...
            if (data.history && Array.isArray(data.history)) {
              data.history.forEach((h) => DB.addHistory(h));
          ...
          status.innerHTML =
            '<span style="color:var(--green)">✓ Imported ' +
            count +
            " bookmarks from file</span>";
          showToast("✓ Imported " + count + " bookmarks");
          renderBookmarksPanel();
          renderHistoryPanel();
        } catch (err) {
          status.innerHTML =
            '<span style="color:var(--red)">✗ Greška: ' +
            err.message +
            "</span>";
        }
      };
  ...
  document.getElementById("btnExportData")?.addEventListener("click", () => {
    const data = {
      ...
      extensions: JSON.parse(localStorage.getItem("ex_extensions") || "[]"),
    ...
    a.download =
      "etherx-export-" + new Date().toISOString().slice(0, 10) + ".json";
    a.click();
...
(function initAiAgent() {
  ...
  // ── AI Agent Core Logic ──
  async function getSmartResponse(m) {
    if (
      m.includes("memorija") ||
      m.includes("potrošnja") ||
      m.includes("ram")
    ) {
    ...
    if (
      ...
      m.includes("provjeri") ||
      m.includes("health") ||
      m.includes("dijagnostik") ||
      m.includes("system") ||
      m.includes("provjera")
    ) {
      const checks = checkSystemHealth();
      let resp =
        "📊 **Provjera sustava EtherX Browser:**\n\n" + checks.join("\n");
      // Also check kriptoentuzijasti.io
      const kOk = await checkKriptoSite();
      resp +=
        "\n" +
        (kOk
          ? "\u2705 kriptoentuzijasti.io: Dostupan"
          : "\u274c kriptoentuzijasti.io: Nedostupan");
      resp +=
        "\n\n🟢 Sustav " +
        (checks.filter((c) => c.startsWith("\u274c")).length === 0
          ? "radi ispravno!"
          : "ima problema - provjeri crvene stavke.");
      return resp;
    }
    ...
    if (
      ...
      m.includes("pomozi") ||
      m.includes("help") ||
      m.includes("sto mozes") ||
      m.includes("sta mozes") ||
      m.includes("mogucnosti")
    ) {
    ...
    if (
      ...
      if (posts && posts.length) {
        ...
        posts.forEach((p, i) => {
          ...
          const date = p.date
            ? new Date(p.date).toLocaleDateString("hr-HR")
            : "";
          resp +=
            i +
            1 +
    ...
    const cryptoKeywords = {
      ...
      cijena:
        "📈 Za pracenje cijena posjeti CoinGecko ili CoinMarketCap u EtherX pregledniku!",
    };
    ...
    if (searchTerms.length > 3) {
      ...
      if (broader && broader.length) {
        let resp =
          "🤔 Nisam nasao tocan odgovor, ali evo povezanih clanaka s kriptoentuzijasti.io:\n";
  ...
  function addMsg(text, type) {
    ...
    html = html.replace(
      /(https?:\/\/[^\s<]+)/g,
      '<a href="#" onclick="navigateTo(\'$1\');return false" style="color:var(--accent);text-decoration:underline">$1</a>',
    );
    d.innerHTML =
      html +
      '<span class="ai-time">' +
  ...
  async function send() {
    ...
    try {
      ...
      typing.remove();
      addMsg(
        "⚠️ Došlo je do greške: " + err.message + ". Pokušaj ponovo.",
        "bot",
      );
    }
  }
  ...
  clearBtn?.addEventListener("click", () => {
    ...
    msgs.innerHTML =
      '<div class="ai-msg bot">🤖 Chat ociscen. Pitaj me bilo sto o kripto svijetu ili reci "provjeri sustav"!<span class="ai-time">AI Agent</span></div>';
  ...
  if (!hist.length) {
    addMsg(
      "🤖 **Dobrodosli u EtherX AI Agent!**\n\nJa sam tvoj inteligentni asistent. Mogu:\n\u2022 🔍 Pretraziti clanke s kriptoentuzijasti.io\n\u2022 🔧 Provjeriti stanje sustava\n\u2022 📊 Dati informacije o kriptovalutama\n\u2022 📰 Pokazati najnovije vijesti\n\nSto te zanima?",
      "bot",
    );
  }
})();
// ── Edit Menu Handlers ──
document.getElementById("mi-undo")?.addEventListener("click", () => {
  document.execCommand("undo");
  showToast("↩ Undo");
});
document.getElementById("mi-redo")?.addEventListener("click", () => {
  document.execCommand("redo");
  showToast("↪ Redo");
});
document.getElementById("mi-cut")?.addEventListener("click", () => {
...
document.getElementById("mi-paste-match")?.addEventListener("click", () => {
  navigator.clipboard?.readText().then((txt) => {
    document.execCommand("insertText", false, txt);
    showToast("📄 Pasted (plain)");
  });
});
document.getElementById("mi-delete")?.addEventListener("click", () => {
  document.execCommand("delete");
  showToast("🗑 Delete");
});
document.getElementById("mi-select-all")?.addEventListener("click", () => {
  document.execCommand("selectAll");
  showToast("☐ Select All");
});
document.getElementById("mi-autofill-form")?.addEventListener("click", () => {
  showToast("✏️ AutoFill: Forms auto-filled");
});
document.getElementById("mi-distraction")?.addEventListener("click", () => {
  ...
  if (on) {
    ...
    showToast("🛑 Distraction Control ON");
  } else {
    document.getElementById("menuBar").style.display = "";
    document.querySelector(".status-bar").style.display = "";
    showToast("🛑 Distraction Control OFF");
  }
});
document.getElementById("mi-spelling")?.addEventListener("click", () => {
  showToast("Abc Spelling & Grammar check active");
});
document.getElementById("mi-substitutions")?.addEventListener("click", () => {
  showToast("↻ Substitutions enabled");
});
document.getElementById("mi-transformations")?.addEventListener("click", () => {
  showToast("Abc Transformations ready");
});
document.getElementById("mi-speech")?.addEventListener("click", () => {
  if ("speechSynthesis" in window) {
    const u = new SpeechSynthesisUtterance("EtherX Browser is ready.");
    speechSynthesis.speak(u);
    showToast("🗣 Speech started");
  } else showToast("Speech not supported");
});
document.getElementById("mi-autofill")?.addEventListener("click", () => {
  showToast("✏️ AutoFill menu");
});
document.getElementById("mi-dictation")?.addEventListener("click", () => {
  showToast("🎙 Dictation not available in browser mode");
});
document.getElementById("mi-emoji")?.addEventListener("click", () => {
  showToast(
    "😀 Emoji panel: Use your OS emoji picker (Win+. or Ctrl+Cmd+Space)",
  );
});
...
(function initPasswords() {
  ...
  function savePasswords(p) {
    localStorage.setItem(PWD_KEY, JSON.stringify(p));
  }
  function renderPwdList() {
    ...
    if (passwords.length === 0) {
      pwdList.innerHTML =
        '<div style="text-align:center;padding:16px;color:var(--text3);font-size:12px">No saved passwords yet</div>';
      return;
    }
    passwords.forEach((p, i) => {
      const entry = document.createElement("div");
      entry.className = "pwd-entry";
      entry.innerHTML =
        '<div><div class="pwd-entry-site">' +
        p.site +
        '</div><div class="pwd-entry-user">' +
        p.username +
        '</div><div class="pwd-entry-pass">' +
        "•".repeat(p.password.length) +
        '</div></div><div class="pwd-entry-actions"><button title="Show" data-action="show" data-idx="' +
        i +
        '">👁</button><button title="Copy" data-action="copy" data-idx="' +
        i +
        '">📋</button><button title="Delete" data-action="del" data-idx="' +
        i +
        '">🗑</button></div>';
    ...
    pwdList.querySelectorAll("button[data-action]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (btn.dataset.action === "show") {
          const passEl = btn
            .closest(".pwd-entry")
            .querySelector(".pwd-entry-pass");
          if (passEl.textContent.includes("•"))
            passEl.textContent = passwords[idx].password;
  ...
  function unlock() {
    ...
    authInput.parentElement.querySelector(".pwd-subtitle")?.remove();
    authInput.style.display = "none";
    unlockBtn.style.display = "none";
    renderPwdList();
    showToast("🔓 Passwords unlocked");
  }
  document
    .getElementById("sOpenPasswords")
    ?.addEventListener("click", openPwdModal);
    ...
    bioBtn.addEventListener("click", async () => {
      try {
        if (!savedCred) {
          ...
          const assertion = await navigator.credentials.get({
            publicKey: {
              ...
              allowCredentials: [
                {
                  id: Uint8Array.from(
                    atob(savedCred.replace(/-/g, "+").replace(/_/g, "/")),
                    (c) => c.charCodeAt(0),
                  ),
                  type: "public-key",
                },
          ...
          if (assertion) {
            unlock();
            showToast("🔐 Biometric unlock successful");
          }
        }
      } catch (err) {
  ...
  addBtn?.addEventListener("click", () => {
    ...
    if (!password) return;
    const passwords = getPasswords();
    passwords.push({ site, username, password, ts: Date.now() });
    savePasswords(passwords);
    renderPwdList();
...
(function initIconVisibility() {
  document
    .querySelectorAll("#stab-appearance .toggle[data-icon]")
    .forEach((el) => {
...
(function initTitleBarVisibility() {
  document
    .querySelectorAll("#stab-appearance .toggle[data-titlebar]")
    .forEach((el) => {
      ...
      if (!target) return;
      const parentDiv =
        targetId === "titleProfileBtn" ? target.parentElement : null;
      if (saved === false) {
        el.classList.remove("on");
      ...
      el.addEventListener("click", () => {
        ...
        else target.style.display = on ? "" : "none";
  ...
  function renderMessages() {
    ...
    if (collectedMessages.length === 0) {
      messagesEl.innerHTML =
        '<div class="tkai-empty">Nema poruka.<br>Klikni "Skeniranje ON" dok si<br>na TikTok Live.</div>';
      return;
    }
    messagesEl.innerHTML = "";
...
Odgovori SAMO s ${count} prijedloga odgovora, svaki u zasebnom redu. Bez numeracije, bez objašnjenja.`;
  ...
  async function generateReplies(msgs) {
    ...
    if (genBtn) {
      ...
      genBtn.textContent = "⏳ Generiram…";
    }
    if (genAllBtn) {
      genAllBtn.disabled = true;
    }
    repliesEl.innerHTML =
      '<div class="tkai-status"><span class="tkai-scanning-dot"></span>AI generira odgovore…</div>';
...
(function initAutoUpdater() {
  ...
  const UPDATE_MANIFEST_URL =
    "https://kriptoentuzijasti.io/etherx-update/manifest.json";
  ...
  function syncUI() {
    ...
    [$id("updVerSpan"), $id("updInfoCurrent"), $id("devVerDisplay")].forEach(
      (el) => {
        if (el) el.textContent = cur;
      },
    );
    if ($id("updInfoLatest"))
      $id("updInfoLatest").textContent = latestInfo
        ? latestInfo.version
        : updateState.latestVersion || "—";
    if ($id("updInfoLastCheck"))
      $id("updInfoLastCheck").textContent = updateState.lastCheck
        ? new Date(updateState.lastCheck).toLocaleString("hr-HR")
        : "—";
    if ($id("devBuildDate")) $id("devBuildDate").textContent = BUILD_DATE;
    if ($id("devUA"))
      $id("devUA").textContent = navigator.userAgent.slice(0, 80) + "…";
    if ($id("devLastUpdate"))
      $id("devLastUpdate").textContent = updateState.lastInstalled
        ? new Date(updateState.lastInstalled).toLocaleString("hr-HR")
        : "—";
  }
  ...
  async function checkForUpdates(silent = false) {
    ...
    if ($id("devUpdateStatus"))
      $id("devUpdateStatus").textContent = "⏳ Provjerava…";
    if ($id("updBtnCheck")) {
      $id("updBtnCheck").disabled = true;
      $id("updBtnCheck").textContent = "⏳ Provjera…";
    }
    ...
    try {
      ...
      if (hasUpdate) {
        ...
        if ($id("devUpdateStatus"))
          $id("devUpdateStatus").style.color = "var(--accent)";
        if ($id("devUpdateStatus"))
          $id("devUpdateStatus").textContent =
            "🆕 v" + manifest.version + " dostupno";
        ...
        if (manifest.changelog && $id("updChangelogBody")) {
          $id("updChangelogBody").innerHTML = manifest.changelog
            .split("\n")
            .map((l) =>
              l.startsWith("-") ? "<li>" + l.slice(1).trim() + "</li>" : l,
            )
            .join("<br>")
            .replace(
              /(<li>.*<\/li>)/g,
              '<ul style="margin:4px 0 4px 12px">$1</ul>',
            );
        ...
        if (!silent)
          showToast("🆕 EtherX ažuriranje dostupno: v" + manifest.version);
        ...
        if ($id("devUpdateStatus")) {
          $id("devUpdateStatus").style.color = "var(--green)";
          $id("devUpdateStatus").textContent = "✅ Ažuran";
        }
        if ($id("updBtnInstall")) $id("updBtnInstall").style.display = "none";
        if ($id("updBtnChangelog"))
          $id("updBtnChangelog").style.display = "none";
        if (!silent)
          showToast("✅ EtherX Browser je ažuran (v" + CURRENT_VERSION + ")");
      }
    } catch (err) {
      setBadge("error", "❌ Greška pri provjeri");
      if ($id("devUpdateStatus")) {
        $id("devUpdateStatus").style.color = "var(--red)";
        $id("devUpdateStatus").textContent = "❌ " + (err.message || "Greška");
      }
      if (!silent)
        showToast("⚠️ Nije moguće provjeriti ažuriranja: " + err.message, 4000);
      console.warn("[AutoUpdater] check failed:", err);
    } finally {
      if ($id("updBtnCheck")) {
        $id("updBtnCheck").disabled = false;
        $id("updBtnCheck").textContent = "🔍 Provjeri ažuriranja";
      }
    }
  }
  ...
  async function downloadAndInstall(info) {
    ...
    try {
      ...
      while (true) {
        ...
        if (contentLength > 0)
          setProgress(
            ...
            "Preuzimanje… " + Math.round(received / 1024) + " KB",
          );
        else
          setProgress(
            Math.min(80, Math.round((received / 512000) * 80)),
            "Preuzimanje… " + Math.round(received / 1024) + " KB",
          );
      }
      ...
      if ($id("updBtnInstall")) $id("updBtnInstall").style.display = "none";
      showToast(
        '✅ Ažuriranje preuzeto! Klikni "Restart i primijeni" za primjenu.',
        5000,
      );
    } catch (err) {
  ...
  function restartBrowser() {
    ...
    setTimeout(() => {
      if (
        window.electronAPI &&
        typeof window.electronAPI.relaunch === "function"
      ) {
  ...
  $id("updBtnInstall")?.addEventListener("click", () => {
    if (latestInfo) downloadAndInstall(latestInfo);
  });
  ...
  $id("devBtnCheckUpdate")?.addEventListener("click", () =>
    checkForUpdates(false),
  );
  $id("devBtnOpenUpdateTab")?.addEventListener("click", () => {
    // Switch to Updates tab in settings
    document
      .querySelectorAll(".sit-btn")
      .forEach((b) => b.classList.remove("active"));
    document
      .querySelectorAll(".s-tab-pane")
      .forEach((p) => p.classList.remove("active"));
    document
      .querySelector('.sit-btn[data-stab="updates"]')
      ?.classList.add("active");
...
const LANGS = {
  en: {
    ...
    "lp-general": "General",
    "lp-appearance": "Appearance",
    "lp-language": "Language",
    "lp-newTab": "New Tab",
    "lp-closeTab": "Close Tab",
    "lp-bookmarkPage": "Bookmark Page",
    menuFile: "File",
    menuEdit: "Edit",
    menuView: "View",
    menuHistory: "History",
    ...
    menuHelp: "Help",
    miNewTab: "New Tab",
    miNewPrivate: "New Private Window",
    miOpenLocation: "Open Location…",
    miCloseTab: "Close Tab",
    miPrint: "Print…",
    miUndo: "Undo Typing",
    miRedo: "Redo",
    miCopy: "Copy",
    ...
    miZoomIn: "Zoom In",
    miZoomOut: "Zoom Out",
    miZoomReset: "Actual Size",
    miFullscreen: "Enter Full Screen",
    miCustomiseToolbar: "Customize Toolbar…",
    miBack: "Back",
    miForward: "Forward",
    miReload: "Reload Page",
    miHardReload: "Hard Reload",
    miStop: "Stop",
    miBookmarkAll: "Bookmark All Tabs",
    miShowBookmarks: "Show Bookmarks",
    miShowHistory: "Show History",
    miShowDownloads: "Show Downloads",
    miAbout: "About EtherX Browser",
  ...
  hr: {
    ...
    "lp-view": "Prikaz",
    "lp-history": "Povijest",
    "lp-bookmarks": "Zabilješke",
    "lp-settings": "Postavke",
    "lp-general": "Općenito",
    "lp-appearance": "Izgled",
    "lp-language": "Jezik",
    "lp-newTab": "Nova kartica",
    "lp-closeTab": "Zatvori karticu",
    "lp-bookmarkPage": "Označi stranicu",
    menuFile: "Datoteka",
    menuEdit: "Uredi",
    menuView: "Prikaz",
    menuHistory: "Povijest",
    menuBookmarks: "Zabilješke",
    menuDevelop: "Razvoj",
    menuWindow: "Prozor",
    menuHelp: "Pomoć",
    miNewTab: "Nova kartica",
    miNewPrivate: "Novi privatni prozor",
    miOpenLocation: "Otvori lokaciju…",
    miCloseTab: "Zatvori karticu",
    miPrint: "Ispis…",
    miUndo: "Poništi unos",
    miRedo: "Ponovi",
    miCopy: "Kopiraj",
    miPaste: "Zalijepi",
    miSelectAll: "Odaberi sve",
    miFind: "Traži na stranici",
    miZoomIn: "Povećaj",
    miZoomOut: "Smanji",
    miZoomReset: "Stvarna veličina",
    miFullscreen: "Uđi u cijeli zaslon",
    miCustomiseToolbar: "Prilagodi alatnu traku…",
    miBack: "Natrag",
    miForward: "Naprijed",
    miReload: "Osvježi stranicu",
    miHardReload: "Jako osvježi",
    miStop: "Zaustavi",
    miBookmarkAll: "Označi sve kartice",
    miShowBookmarks: "Prikaži zabilješke",
    miShowHistory: "Prikaži povijest",
    miShowDownloads: "Prikaži preuzimanja",
    miAbout: "O EtherX Browseru",
    settingsTitle: "Postavke",
    tabGeneral: "Općenito",
    tabTabs: "Kartice",
    tabAutofill: "AutoIspuna",
    tabPasswords: "Lozinke",
    tabSearch: "Pretraga",
    tabSecurity: "Sigurnost",
    tabPrivacy: "Privatnost",
    tabWebsites: "Web-stranice",
    tabProfiles: "Profili",
    tabExtensions: "Proširenja",
    tabAdvanced: "Napredno",
    tabDeveloper: "Programer",
    tabFlags: "Zastavice",
    tabMigration: "Uvoz podataka",
    tabAppearance: "Izgled",
    tabLanguage: "Jezik",
  },
  de: {
    name: "Deutsch",
    ...
    "lp-view": "Ansicht",
    "lp-history": "Verlauf",
    "lp-bookmarks": "Lesezeichen",
    "lp-settings": "Einstellungen",
    "lp-general": "Allgemein",
    "lp-appearance": "Darstellung",
    "lp-language": "Sprache",
    "lp-newTab": "Neuer Tab",
    "lp-closeTab": "Tab schließen",
    "lp-bookmarkPage": "Seite merken",
    menuFile: "Datei",
    menuEdit: "Bearbeiten",
    menuView: "Ansicht",
    menuHistory: "Verlauf",
    ...
    menuHelp: "Hilfe",
    miNewTab: "Neuer Tab",
    miNewPrivate: "Neues privates Fenster",
    miOpenLocation: "Adresse öffnen…",
    miCloseTab: "Tab schließen",
    miPrint: "Drucken…",
    miUndo: "Eingabe rückgängig",
    miRedo: "Wiederholen",
    miCopy: "Kopieren",
    miPaste: "Einfügen",
    miSelectAll: "Alles auswählen",
    miFind: "Auf Seite suchen",
    miZoomIn: "Vergrößern",
    miZoomOut: "Verkleinern",
    miZoomReset: "Tatsächliche Größe",
    miFullscreen: "Vollbild",
    miCustomiseToolbar: "Symbolleiste anpassen…",
    miBack: "Zurück",
    miForward: "Vorwärts",
    miReload: "Seite laden",
    miHardReload: "Vollständig laden",
    miStop: "Stopp",
    miBookmarkAll: "Alle Tabs merken",
    miShowBookmarks: "Lesezeichen anzeigen",
    miShowHistory: "Verlauf anzeigen",
    miShowDownloads: "Downloads anzeigen",
    miAbout: "Über EtherX Browser",
    settingsTitle: "Einstellungen",
    tabGeneral: "Allgemein",
    tabTabs: "Tabs",
    tabAutofill: "AutoAusfüllen",
    tabPasswords: "Passwörter",
    tabSearch: "Suche",
    tabSecurity: "Sicherheit",
    tabPrivacy: "Datenschutz",
    tabWebsites: "Webseiten",
    tabProfiles: "Profile",
    tabExtensions: "Erweiterungen",
    tabAdvanced: "Erweitert",
    tabDeveloper: "Entwickler",
    tabFlags: "Feature-Flags",
    tabMigration: "Daten importieren",
  ...
  it: {
    ...
    "lp-edit": "Modifica",
    "lp-view": "Visualizza",
    "lp-history": "Cronologia",
    "lp-bookmarks": "Preferiti",
    "lp-settings": "Impostazioni",
    "lp-general": "Generale",
    "lp-appearance": "Aspetto",
    "lp-language": "Lingua",
    "lp-newTab": "Nuova scheda",
    "lp-closeTab": "Chiudi scheda",
    "lp-bookmarkPage": "Aggiungi ai preferiti",
    menuFile: "File",
    menuEdit: "Modifica",
    menuView: "Visualizza",
    menuHistory: "Cronologia",
    menuBookmarks: "Preferiti",
    menuDevelop: "Sviluppo",
    menuWindow: "Finestra",
    menuHelp: "Aiuto",
    miNewTab: "Nuova scheda",
    miNewPrivate: "Nuova finestra privata",
    miOpenLocation: "Apri posizione…",
    miCloseTab: "Chiudi scheda",
    miPrint: "Stampa…",
    miUndo: "Annulla digitazione",
    miRedo: "Ripeti",
    miCopy: "Copia",
    miPaste: "Incolla",
    miSelectAll: "Seleziona tutto",
    miFind: "Trova nella pagina",
    miZoomIn: "Ingrandisci",
    miZoomOut: "Riduci",
    miZoomReset: "Dimensione effettiva",
    miFullscreen: "Schermo intero",
    miCustomiseToolbar: "Personalizza barra…",
    miBack: "Indietro",
    miForward: "Avanti",
    miReload: "Ricarica pagina",
    miHardReload: "Ricarica completa",
    miStop: "Stop",
    miBookmarkAll: "Aggiungi tutte le schede",
    miShowBookmarks: "Mostra preferiti",
    miShowHistory: "Mostra cronologia",
    miShowDownloads: "Mostra download",
    miAbout: "Informazioni su EtherX Browser",
    settingsTitle: "Impostazioni",
    tabGeneral: "Generale",
    tabTabs: "Schede",
    tabAutofill: "Compilazione automatica",
    tabPasswords: "Password",
    tabSearch: "Ricerca",
    tabSecurity: "Sicurezza",
    tabPrivacy: "Privacy",
    tabWebsites: "Siti web",
    tabProfiles: "Profili",
    tabExtensions: "Estensioni",
    tabAdvanced: "Avanzate",
    tabDeveloper: "Sviluppatore",
    tabFlags: "Funzionalità",
    tabMigration: "Importa dati",
  ...
  fr: {
    ...
    "lp-edit": "Éditer",
    "lp-view": "Affichage",
    "lp-history": "Historique",
    "lp-bookmarks": "Favoris",
    "lp-settings": "Réglages",
    "lp-general": "Général",
    "lp-appearance": "Apparence",
    "lp-language": "Langue",
    "lp-newTab": "Nouvel onglet",
    "lp-closeTab": "Fermer l'onglet",
    "lp-bookmarkPage": "Mettre en favori",
    menuFile: "Fichier",
    menuEdit: "Éditer",
    menuView: "Affichage",
    ...
    menuWindow: "Fenêtre",
    menuHelp: "Aide",
    miNewTab: "Nouvel onglet",
    miNewPrivate: "Nouvelle fenêtre privée",
    miOpenLocation: "Ouvrir l'adresse…",
    miCloseTab: "Fermer l'onglet",
    miPrint: "Imprimer…",
    miUndo: "Annuler la saisie",
    miRedo: "Rétablir",
    miCopy: "Copier",
    miPaste: "Coller",
    miSelectAll: "Tout sélectionner",
    miFind: "Rechercher dans la page",
    miZoomIn: "Agrandir",
    miZoomOut: "Réduire",
    miZoomReset: "Taille réelle",
    miFullscreen: "Plein écran",
    miCustomiseToolbar: "Personnaliser la barre…",
    miBack: "Précédent",
    miForward: "Suivant",
    miReload: "Recharger la page",
    miHardReload: "Rechargement complet",
    miStop: "Arrêter",
    miBookmarkAll: "Marquer tous les onglets",
    miShowBookmarks: "Afficher les favoris",
    miShowHistory: "Afficher l'historique",
    miShowDownloads: "Afficher les téléchargements",
    miAbout: "À propos d'EtherX Browser",
    settingsTitle: "Réglages",
    tabGeneral: "Général",
    tabTabs: "Onglets",
    tabAutofill: "Remplissage auto",
    tabPasswords: "Mots de passe",
    tabSearch: "Recherche",
    tabSecurity: "Sécurité",
    tabPrivacy: "Confidentialité",
    tabWebsites: "Sites web",
    tabProfiles: "Profils",
    tabExtensions: "Extensions",
    tabAdvanced: "Avancé",
    tabDeveloper: "Développeur",
    tabFlags: "Indicateurs",
    tabMigration: "Importer des données",
...
const LANG_MAP = {
  menuFile: '[data-menu="file"]',
  menuEdit: '[data-menu="edit"]',
  menuView: '[data-menu="view"]',
  menuHistory: '[data-menu="history"]',
  menuBookmarks: '[data-menu="bm"]',
  menuDevelop: '[data-menu="develop"]',
  menuWindow: '[data-menu="window"]',
  menuHelp: '[data-menu="help"]',
  miNewTab: "#mi-new-tab",
  miNewPrivate: "#mi-new-private",
  miOpenLocation: "#mi-open-location",
  miCloseTab: "#mi-close-tab",
  miPrint: "#mi-print",
  miUndo: "#mi-undo",
  miRedo: "#mi-redo",
  miCopy: "#mi-copy",
  miPaste: "#mi-paste",
  miSelectAll: "#mi-select-all",
  miFind: "#mi-find",
  miZoomIn: "#mi-zoom-in",
  miZoomOut: "#mi-zoom-out",
  miZoomReset: "#mi-zoom-reset",
  miFullscreen: "#mi-fullscreen",
  miCustomiseToolbar: "#mi-customise-toolbar",
  miBack: "#mi-back",
  miForward: "#mi-forward",
  miReload: "#mi-reload",
  miHardReload: "#mi-hard-reload",
  miStop: "#mi-stop",
  miAbout: "#mi-about",
  settingsTitle:
    '.settings-panel-title-text, .panel-title[data-panel="settings"]',
};
    ...
    document.querySelectorAll(sel).forEach((el) => {
      ...
      if (el.dataset && el.dataset.menu !== undefined) {
        ...
        el.childNodes.forEach((n) => {
          if (n.nodeType === 3) n.textContent = lang[key] + " ";
        });
...
(function initLanguage() {
  ...
  sel.addEventListener("change", () => {
    ...
    showToast(
      LANGS[sel.value]?.flag + " " + LANGS[sel.value]?.name + " applied",
    );
  });
  ...
  document.getElementById("langDownloadBtn")?.addEventListener("click", () => {
    ...
    if (status) {
      ...
      status.textContent = "⏳ Downloading " + LANGS[code]?.name + " pack…";
    }
    setTimeout(() => {
      DB.saveSetting("langPack_" + code, true);
      if (status) {
        status.textContent =
          "✅ " + LANGS[code]?.name + " language pack cached offline.";
      }
...
const CTT_BUTTONS = [
  ...
  {
    id: "btnExtensions",
    key: "showBtnExtensions",
    icon: "🧩",
    name: "Extensions",
  },
  {
    id: "btnBookmarks",
    key: "showBtnBookmarks",
    icon: "⭐",
    name: "Bookmarks",
  },
...
function openCustomToolbar() {
  ...
  CTT_BUTTONS.forEach((def) => {
    ...
    const liveIcon = btn
      ? btn.textContent.trim().charAt(0) || def.icon
      : def.icon;
    ...
    tile.addEventListener("click", () => {
      ...
      const settingToggle = document.querySelector(
        `#stab-appearance .toggle[data-setting="${def.key}"]`,
      );
      if (settingToggle) {
        newOn
          ? settingToggle.classList.add("on")
          : settingToggle.classList.remove("on");
      }
    });
...
document
  .getElementById("customToolbarOverlay")
...
(function initCustomSearch() {
  ...
  if (customUrl) {
    const inp = document.getElementById("sCustomSearchURL");
    if (inp) inp.value = customUrl;
  }
})();
...
(function initProfileDropdown() {
  ...
  function getProfiles() {
    return [{ name: "Default", ts: 0 }].concat(
      JSON.parse(localStorage.getItem("ex_profiles") || "[]"),
    );
  }
  function getActive() {
  ...
  function render() {
    ...
    profiles.forEach((p) => {
      const item = document.createElement("div");
      item.className = "pd-item" + (p.name === active ? " active" : "");
      item.innerHTML = (p.name === active ? "✓ " : " ") + p.name;
      item.addEventListener("click", () => {
        setActive(p.name);
        dd.classList.remove("open");
        showToast("👤 Profile: " + p.name);
        render();
      });
      dd.appendChild(item);
    });
    const sep = document.createElement("div");
    sep.className = "pd-sep";
    dd.appendChild(sep);
    const manage = document.createElement("div");
    manage.className = "pd-item";
  ...
  if (oldAbout) {
    oldAbout.addEventListener("click", () => {
      ...
      if (isIOS) installMsg = "📱 iOS: Tap Share → Add to Home Screen";
        ...
        installMsg =
          "🖥 Mac: Use Ctrl+Cmd+A or install via Chrome → ⋮ → Install EtherX";
      else if (isWin)
        installMsg = "🖥 Windows: Click ⊕ in address bar or ⋮ → Install EtherX";
...
(function restoreSession() {
  ...
  const saved =
    ...
    localStorage.getItem("ex_session_tabs");
  const activeIdStr =
    localStorage.getItem("ex_session_active_" + windowId) ||
    localStorage.getItem("ex_session_active") ||
    "0";
  ...
  if (saved) {
    try {
      ...
      if (Array.isArray(tabs) && tabs.length) {
        ...
        consoleLog(
          "info",
          "🚀 EtherX Browser initialized – restored " + tabs.length + " tab(s)",
        );
  ...
  createTab();
  renderQuickLinks();
  consoleLog("info", "🚀 EtherX Browser initialized");
  consoleLog("success", "⬡ Web3 provider: window.ethereum injected");
})();
...
function openShareSheet() {
  ...
  const actions = [
    ...
    },
    {
      ...
      fn: () => {
        closeShareSheet();
        document.getElementById("btnBookmarkPage")?.click();
      },
    },
    {
      icon: "📤",
...
document
  ...
  .addEventListener("click", closeShareSheet);
document.getElementById("shareSheet").addEventListener("click", (e) => {
  if (e.target === document.getElementById("shareSheet")) closeShareSheet();
});
...
function openWindowSwitcher() {
  ...
  STATE.tabs.forEach((tab) => {
    const card = document.createElement("div");
    card.className =
      "wsw-tab-card" + (tab.id === STATE.activeTabId ? " active-tab" : "");
    const favHtml = tab.faviconUrl
...
function closeWindowSwitcher() {
  document.getElementById("windowSwitcher").classList.remove("show");
}
document
  .getElementById("btnWindowSwitcher")
  .addEventListener("click", openWindowSwitcher);
document
  .getElementById("btnCryptoPrice")
  .addEventListener("click", () => togglePanel("cryptoPricePanel"));
document
  .getElementById("wswClose")
  .addEventListener("click", closeWindowSwitcher);
document.getElementById("windowSwitcher").addEventListener("click", (e) => {
  if (e.target === document.getElementById("windowSwitcher"))
    closeWindowSwitcher();
});
document.getElementById("wswNewTab").addEventListener("click", () => {
  createTab();
  closeWindowSwitcher();
});
...
(function initLogoCustomization() {
  ...
  document.getElementById("logoEmojiBtn")?.addEventListener("click", () => {
    emojiInput.style.display =
      emojiInput.style.display === "none" ? "" : "none";
    emojiInput.focus();
  });
...
document
  .getElementById("mi-screenshot")
  ?.addEventListener("click", () => takeScreenshot(false));
document
  .getElementById("mi-screenshot-full")
  ?.addEventListener("click", () => takeScreenshot(true));
...
(function initScreenshotSettings() {
  ...
  if (locSel) {
    ...
    locSel.addEventListener("change", () => {
      ...
      showToast(
        "📷 Lokacija screenshota: " + locSel.options[locSel.selectedIndex].text,
      );
    });
  }
})();
    ...
    chooseBtn.addEventListener("click", async () => {
      if (window.etherx?.app?.chooseIcon) {
        ...
        if (result?.ok && result.filePath) {
          ...
          if (preview) {
            preview.src = "file://" + result.filePath;
            preview.style.display = "block";
          }
...
const STORE_DEFS = [
  ...
  {
    id: "storage_cfg",
    label: "🗺️ Storage Config",
    defaultKey: "ex_storage_cfg",
  },
];

function refreshStorageUsage() {
  const t = document.getElementById("storageUsageTable");
  if (!t) return;
  let totalBytes = 0;
  ...
  const other = Object.keys(localStorage).filter(
    (k) => !STORE_DEFS.map((s) => DB.getStorageKey(s.id)).includes(k),
  );
  other.forEach((k) => {
  ...
  t.innerHTML =
    ...
    rows
      .map(
        (r) => `<tr style="border-bottom:1px solid rgba(255,255,255,.04)">
...
function initStorageTab() {
  // Key mapping table
  const kt = document.getElementById("storageKeyTable");
  if (!kt) return;
  ...
  // Export/Import list
  const el = document.getElementById("storeExportList");
  if (!el) return;
  el.innerHTML = STORE_DEFS.map(
    (s) => `
  ...
  // Clear list
  const cl = document.getElementById("storeClearList");
  if (!cl) return;
  cl.innerHTML = STORE_DEFS.map(
    (s) => `
...
function _migrateStore(storeId) {
  ...
  if (oldKey === newKey) {
    showToast("⚠️ Keys are identical — nothing to migrate");
    return;
  }
  const data = localStorage.getItem(oldKey);
  if (data === null) {
    showToast("⚠️ No data found at key: " + oldKey);
    return;
  }
...
function _exportStore(storeId) {
  ...
  if (!data) {
    showToast("⚠️ No data to export for: " + key);
    return;
  }
  const def = STORE_DEFS.find((s) => s.id === storeId);
  const filename =
    "etherx-" + storeId + "-" + new Date().toISOString().slice(0, 10) + ".json";
  ...
  input.onchange = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (
          !confirm(
            'Import into "' + key + '"? This will REPLACE existing data.',
          )
        )
          return;
        ...
        showToast("⚠️ Invalid JSON file: " + err.message);
      }
    };
    reader.readAsText(file);
  };
...
document.getElementById("sChangeMasterPwd")?.addEventListener("click", () => {
  ...
  if (!newPwd || newPwd.length < 4) {
    showToast("⚠️ New password must be at least 4 characters");
    return;
  }
...
function initDownloadsTab() {
  ...
  ["Linux", "Windows", "MacIntel", "MacArm", "Android", "IOS"].forEach(
    (platform) => {
...
document.getElementById("saveDownloadLinks")?.addEventListener("click", () => {
  ...
  ["Linux", "Windows", "MacIntel", "MacArm", "Android", "IOS"].forEach(
    (platform) => {
      const el = document.getElementById("dl" + platform);
      if (el && el.value.trim())
        links[platform.toLowerCase()] = el.value.trim();
    },
  );
...
document.getElementById("btnSaveSettings")?.addEventListener("click", () => {
  Object.keys(PENDING_SETTINGS).forEach((k) =>
    DB.saveSetting(k, PENDING_SETTINGS[k]),
  );
...
function renderSitePermsList() {
  const el = document.getElementById("sitePermsList");
  if (!el) return;
  const perms = getSitePerms();
  const domains = Object.keys(perms);
  if (!domains.length) {
    el.innerHTML =
      '<span style="color:var(--text3);font-size:11px;padding:4px 0">No per-site overrides yet. Changes made via the padlock icon will appear here.</span>';
    return;
  }
  const LABELS = {
    autoPlay: "Auto-Play",
  ...
  el.innerHTML = domains
    .map((domain) => {
      ...
      const pills = Object.entries(d)
        .map(
          ([k, v]) =>
            `<span style="background:var(--bg2);border:1px solid var(--border);border-radius:3px;padding:1px 5px;margin-right:3px;font-size:10px;color:var(--text2)">${LABELS[k] || k}: <strong>${v}</strong></span>`,
        )
        .join("");
...
document
  .querySelector('[data-stab="websites"]')
  ?.addEventListener("click", renderSitePermsList);
...
window.customPrompt = function (message, defaultValue, callback) {
  ...
  box.style.cssText =
    "background:var(--bg); border:1px solid var(--border); padding:20px; border-radius:8px; width:300px; color:var(--text); font-family:sans-serif; box-shadow:0 10px 30px rgba(0,0,0,0.5);";
  ...
  inp.focus();
  document.getElementById("cpCancel").onclick = () => {
    overlay.remove();
    callback(null);
  };
...
window.customConfirm = function (message, callback) {
  ...
  document.getElementById("ccOk").onclick = () => {
    overlay.remove();
    callback(true);
  };
};
  ...
  window.customPrompt("Enter profile name:", "", (name) => {
    ...
    if (pane) {
      ...
      if (grp) {
        ...
        row.className = "s-row";
        row.innerHTML =
          '<div class="s-row-left"><div class="s-row-label">' +
          name.trim() +
          '</div><div class="s-row-desc">Custom profile</div></div><button class="s-btn-sm" style="color:var(--red)">Remove</button>';
        row.querySelector("button").addEventListener("click", function () {
          this.closest(".s-row").remove();
          const pp = JSON.parse(localStorage.getItem("ex_profiles") || "[]");
          localStorage.setItem(
            "ex_profiles",
            JSON.stringify(pp.filter((x) => x.name !== name.trim())),
          );
...
(function loadProfiles() {
  ...
  profiles.forEach((p) => {
    ...
    row.querySelector("button").addEventListener("click", function () {
      ...
      localStorage.setItem(
        "ex_profiles",
        JSON.stringify(pp.filter((x) => x.name !== p.name)),
      );
...
document.getElementById("sSetCurrentPage")?.addEventListener("click", () => {
  const t = getActiveTab();
  if (t?.url) {
    const el = document.getElementById("sHomepage");
    if (el) el.value = t.url;
    DB.saveSetting("homepage", t.url);
    showToast("Homepage set to current page");
  }
});
document.getElementById("sClearHistory")?.addEventListener("click", () => {
  window.customConfirm("Remove all website data?", (c) => {
    if (c) {
      DB.clearHistory();
...
document.getElementById("sClearCache")?.addEventListener("click", () => {
  window.customConfirm(
    "Clear cached images and scripts?\n\n(Clears browser caches via Cache API and service worker caches)",
    (c) => {
      if (!c) return;
      const tasks = [];
      if ("caches" in window) {
      ...
      Promise.all(tasks)
        .then(() => showToast("🗑 Cache cleared"))
        .catch(() => showToast("🗑 Cache cleared (partial)"));
    },
  );
});
document.getElementById("sClearCookies")?.addEventListener("click", () => {
  window.customConfirm(
    "Clear all cookies?\n\n⚠️ You will be logged out of websites.",
    (c) => {
      if (!c) return;
...
document.getElementById("sClearLocalStorage")?.addEventListener("click", () => {
  window.customConfirm(
    "Clear local storage?\n\n⚠️ This will remove saved settings, bookmarks, history, extensions and all browser data.",
    (c) => {
      if (!c) return;
...
document
  .getElementById("sClearSessionStorage")
  ?.addEventListener("click", () => {
...
document.getElementById("sClearAllBrowsing")?.addEventListener("click", () => {
  window.customConfirm(
    "Clear EVERYTHING?\n\n• History\n• Bookmarks\n• Extensions\n• Settings\n• Cookies\n• Cache\n• Session\n• Site permissions\n\n⚠️ This cannot be undone. The page will reload.",
    (c) => {
      if (!c) return;
      // Clear all caches
      if ("caches" in window)
...
document
  .getElementById("sGetExtensions")
  ?.addEventListener("click", () => openExtPanelOnTab("store"));
document
  .getElementById("sOpenExtPanel")
  ?.addEventListener("click", () => openExtPanelOnTab("installed"));
document
  .getElementById("sManageExtensions")
  ?.addEventListener("click", () => openExtPanelOnTab("installed"));
...
function getSitePerms() {
  try {
    return JSON.parse(localStorage.getItem("ex_site_perms") || "{}");
  } catch (e) {
    return {};
  }
}
function saveSitePerms(obj) {
  localStorage.setItem("ex_site_perms", JSON.stringify(obj));
}
function getSitePerm(domain, key) {
  const perms = getSitePerms();
  if (perms[domain] && perms[domain][key] !== undefined)
    return perms[domain][key];
...
document.getElementById("urlIcon").addEventListener("click", () => {
  ...
  if (!url) {
    secIcon.textContent = "🌐";
    secLabel.textContent = "No page loaded";
    secLabel.style.color = "var(--text3)";
  } else if (isHttps) {
    secIcon.textContent = "🔒";
    secLabel.textContent = "Secure connection";
    secLabel.style.color = "var(--green)";
  } else {
    secIcon.textContent = "⚠️";
    secLabel.textContent = "Not secure (HTTP)";
    secLabel.style.color = "var(--yellow)";
  }
  // Populate cert fields
  document.getElementById("sipCertSub").textContent = siteDomain || "—";
  document.getElementById("sipCertIss").textContent = isHttps
    ? "Let's Encrypt / R11"
    : "—";
  ...
  document.getElementById("sipCertFrom").textContent = isHttps
    ? fmt(validFrom)
    : "—";
  document.getElementById("sipCertTo").textContent = isHttps
    ? fmt(validTo)
    : "—";
  document.getElementById("sipCertProto").textContent = isHttps
    ? "TLS 1.3 / AES-128-GCM"
    : "—";
  // Load saved permissions (per-domain or global default) into selects
  const selMap = {
    autoPlay: "sipAutoPlay",
    popups: "sipPopups",
    camera: "sipCamera",
    mic: "sipMic",
    screen: "sipScreen",
  ...
  window.customConfirm(
    ...
    (ok) => {
      ...
      const h = DB.getHistory().filter((x) => {
        try {
          return new URL(x.url).hostname !== domain;
        } catch (e) {
          return true;
        }
      });
...
document.addEventListener("click", (e) => {
  if (
    siteInfoPopup.classList.contains("show") &&
...
function createTabGroup(name, color, tabIds) {
  const g = {
    id: ++groupIdCounter,
    name: name || "Group " + groupIdCounter,
    color:
      color || TAB_GROUP_COLORS[(groupIdCounter - 1) % TAB_GROUP_COLORS.length],
    tabIds: tabIds || [],
  };
  STATE.groups.push(g);
...
function renderTabGroupLabel(g) {
  ...
  label.addEventListener("dblclick", () => {
    window.customPrompt("Rename group:", g.name, (n) => {
      if (n) {
        g.name = n;
        label.querySelector("span:last-child").textContent = n;
      }
    });
  });
  // Insert before first tab that belongs to group, or at end
  const firstTabId = g.tabIds[0];
  const firstTabEl = firstTabId
  ...
  g.tabIds.forEach((tid) => {
    ...
    if (tel) {
      tel.style.setProperty("--group-color", g.color);
      tel.classList.add("in-group");
    }
  });
}
...
document.getElementById("ctx-add-to-group").addEventListener("click", () => {
  if (!STATE.groups.length) {
    showToast("No groups yet — create one first");
    return;
  }
...
document.getElementById("mi-prev-tab-group")?.addEventListener("click", () => {
  ...
  const ci = STATE.groups.findIndex((g) =>
    g.tabIds.includes(STATE.activeTabId),
  );
...
async function aiAutoGroupTabs() {
  if (!STATE.tabs.length) {
    showToast("No tabs to group");
    return;
  }
  if (!window.etherx?.ai?.groupTabs) {
  ...
  try {
    ...
    document
      .querySelectorAll(".tab-group-label[data-ai]")
    ...
    showToast(
      created
        ? `🗂️ Created ${created} AI tab group${created > 1 ? "s" : ""}`
        : '🗂️ All tabs in "Other" — no groups created',
    );
  } catch (e) {
...
document
  .getElementById("btnAiAutoGroup")
  ?.addEventListener("click", aiAutoGroupTabs);
...
document.getElementById("mi-download-app")?.addEventListener("click", () => {
  ...
  if (cf) {
    ...
    document.getElementById("urlInput").value =
      "github.com/ktrucek/etherx-browser-2/releases";
  }
});
...
(function initCookieBanner() {
  ...
  document
    .getElementById("cookieBtnAccept")
    ?.addEventListener("click", () => dismissBanner(true));
  document
    .getElementById("cookieBtnDecline")
    ?.addEventListener("click", () => dismissBanner(false));
})();
...
(function etherxElectronBridge() {
  ...
  document
    .getElementById("phishingClose")
    ?.addEventListener("click", hidePhishingBanner);
  document.getElementById("phishingBack")?.addEventListener("click", () => {
    hidePhishingBanner();
    const tab = getActiveTab();
    const wv = getTabWebview(tab?.id);
    if (wv && window.electronWebview) {
      safeWebviewExecute(wv, tab?.id, "canGoBack")
        .then((canGoBack) => {
          if (canGoBack) {
            safeWebviewExecute(wv, tab?.id, "goBack").catch(() =>
              window.history.back(),
  ...
  document
    .getElementById("phishingProceed")
    ?.addEventListener("click", hidePhishingBanner);
  ...
  if (typeof navigateTo === "function") {
    ...
    const patchedNav = function (raw, tabId) {
      ...
      if (url.startsWith("http"))
        setTimeout(() => afterNavPhishingCheck(url), 500);
      return result;
    };
  ...
  setTimeout(async () => {
    try {
      ...
      if (result?.isNew) {
        showToast(
          `🔄 Nova verzija ${result.latest} dostupna! Postavke → Nadogradnje.`,
        );
      }
    } catch (e) {
      /* silent */
  ...
  // ════════════════════════════════════════════════════════════════════════════
  {
    ...
    async function performBotDetection(force = false) {
      try {
        ...
        if (dismissed && !force) {
          console.log(
            "[EtherX] Bot detection warning permanently dismissed by user",
          );
          return;
        }
        ...
        const shownThisSession =
          sessionStorage.getItem(BOT_SESSION_KEY) === "true";
        ...
        if (result?.isBot && result.reasons?.length) {
          ...
          showToast(
            "⚠️ In-app preglednik detektiran — neke funkcije možda neće raditi ispravno.",
            5000,
          );
          sessionStorage.setItem(BOT_SESSION_KEY, "true");
        } else {
  ...
  const BIO_LOCK = (() => {
    ...
    function _resetInactivityTimer() {
      if (!localStorage.getItem(CRED_KEY) && !localStorage.getItem(PASS_KEY))
        return; // not set up
    ...
    async function unlock() {
      ...
      if (!storedCid && !localStorage.getItem(PASS_KEY)) {
        ...
        try {
          if (hasBio) {
            document.getElementById("bioLockTitle").textContent =
              "Postavi zaključavanje";
            document.getElementById("bioLockSub").textContent =
              "Registrirajte Face ID / Touch ID za zaštitu preglednika.";
            document.getElementById("bioUnlockBtn").textContent =
              "🔓 Postavi Face ID / Touch ID";
    ...
    document
      .getElementById("bioPassphraseBtn")
      ?.addEventListener("click", tryPassphrase);
    document
      .getElementById("bioPassphraseInput")
    ...
    async function stepCameraLiveness() {
      ...
      try {
        ...
        const eX = Math.floor(cw * 0.3),
          eY = Math.floor(ch * 0.22);
        const eW = Math.floor(cw * 0.4),
          eH = Math.floor(ch * 0.2);
        ...
        if (livenessOk) {
          ...
          if (camErrEl) {
            camErrEl.textContent =
              "Pomjerite glavu ili povećajte svjetlost i pokušajte opet.";
        ...
        if (camErrEl) {
          camErrEl.textContent = "Dozvolite pristup kameri i pokušajte ponovo.";
          camErrEl.style.display = "block";
        }
        await wait(2000);
        panel.style.display = "none";
    ...
    document
      ...
      ?.addEventListener("click", async () => {
        ...
        if (ok) {
          ...
          if (errEl) {
            errEl.textContent =
              "Provjera licem nije uspjela. Koristite drugu metodu.";
            errEl.style.display = "block";
            setTimeout(() => {
              if (errEl) errEl.style.display = "none";
            }, 3500);
    ...
    document
      .getElementById("btnBiometricLock")
      ?.addEventListener("click", () => {
        _stopCam();
        const hasCreds =
          localStorage.getItem(CRED_KEY) || localStorage.getItem(PASS_KEY);
        if (!hasCreds) {
          showToast(
            "🔐 Klikni ponovo u lock screenu da postaviš biometrijsko zaključavanje.",
          );
        }
        showLock();
      });
  ...
  // ════════════════════════════════════════════════════════════════════════════
  {
    ...
    document
      .getElementById("locConsentAllow")
      ?.addEventListener("click", () => {
    ...
    document
      .getElementById("locConsentAlwaysAllow")
      ?.addEventListener("click", () => {
        ...
        if (_currentRequestHostname) {
          ...
          showToast(
            "📍 Lokacija uvijek dozvoljena za " + _currentRequestHostname,
          );
        } else if (
          typeof _sipCurrentDomain !== "undefined" &&
          _sipCurrentDomain &&
          _sipCurrentDomain !== "—"
        ) {
    ...
    function requestUserLocation() {
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          showToast(
            `📍 Lokacija dobivena: ${latitude.toFixed(4)}, ${longitude.toFixed(4)} (±${Math.round(accuracy)}m)`,
          );
    ...
    document.getElementById("sipLocation")?.addEventListener(
      "change",
      function (evt) {
      ...
      true,
    ); // capture: true — fire before the auto-save bubbling listener
    ...
    if (window.addEventListener) {
      ...
      document.addEventListener(
        "tabSwitch",
        checkLocationPermissionOnNavigation,
      );
    }
    ...
    (function checkLocationOnStartup() {
      const hasShownLocationPrompt = sessionStorage.getItem(
        "etherx_location_prompt_shown",
      );

    ...
    function switchTab(id) {
      ...
      if (!tab.url) {
        ...
        if (window.electronWebview) {
          if (frame.src !== tab.url && frame.src !== tab.url + '/') frame.src = tab.url;
        } else {
          // Web mode: always update iframe src so switching tabs actually changes content
    ...
    function navigateTo(raw, tabId) {
      ...
      if (tab.id === STATE.activeTabId) {
        ...
        document.getElementById('blockedOverlay').classList.remove('show');
        document.getElementById('readerMode').classList.remove('show');
        setLoading(30); if (window.electronWebview) { frame.src = url; } else { frame.src = '/proxy.php?url=' + encodeURIComponent(url); }
    ...
    document.getElementById('btnBack').addEventListener('click', () => {
      ...
      if (window.electronWebview) { frame.src = u; } else { frame.src = '/proxy.php?url=' + encodeURIComponent(u); } setLoading(25); updateNavBtns(tab); updateTabEl(tab);
    });
    ...
    document.getElementById('btnReload').addEventListener('click', () => {
      const tab = getActiveTab(); if (tab && tab.url) { setLoading(20); if (window.electronWebview && frame.tagName === 'WEBVIEW') { try { frame.reload(); } catch (e) { frame.src = tab.url; } } else { frame.src = '/proxy.php?url=' + encodeURIComponent(tab.url); } consoleLog('log', '↺ Reload: ' + tab.url); }
    });

...
function setupIPC() {
  ...
  /**
   * Summarize the current page via GPT-4o-mini.
   * API key is read from settings (never hard-coded).
   * Results are cached in ai_cache table — same URL never hits the API twice.
   */
  ...
  /** Return all cached summaries (for Settings → AI Cache panel) */
  ipcMain.handle('ai:getCachedSummaries', (_e, limit) => db.getAiCache(limit || 100));

  /** Clear the AI cache */
  ipcMain.handle('ai:clearAiCache', () => db.clearAiCache());

  /** Remove stale entries older than N days */
  ipcMain.handle('ai:pruneAiCache', (_e, days) => db.pruneAiCache(days || 30));
